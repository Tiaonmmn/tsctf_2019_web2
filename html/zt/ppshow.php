...
$rs=query("select * from caicaicms_pp where id='$cpid'");
$row=num_rows($rs);
if(!$row){
showmsg('无记录');
}else{
...
$smallclassname='';
$rsn=query("select classname from caicaicms_zsclass where classzm='".$bigclasszm."'");
$rown=fetch_array($rsn);
$bigclassname=$rown["classname"];
if ($smallclasszm<>""){
$rsn=query("select classname from caicaicms_zsclass where classzm='".$smallclasszm."'");
$rown=fetch_array($rsn);
