...
if ($rowleft){
	while ($rowleft=fetch_array($rsleft)){
		...
		$rsb=query("select count(id) as total from caicaicms_main where editor='".$editor."'and bigclassid='".$rowleft["bigclassid"]."'");
		...
		$rsn=query("select smallclassid from caicaicms_main where editor='".$editor."'and bigclassid='".$rowleft["bigclassid"]."' group by smallclassid");
		$rown=num_rows($rsn);
		if ($rown){
			while ($rown=fetch_array($rsn)){
				$rss=query("select classname,classzm from caicaicms_zsclass where classid='".$rown["smallclassid"]."'");
				$rows=num_rows($rss);
				if ($rows){
				...
				$rss=query("select count(id) as total from caicaicms_main where editor='".$editor."'and smallclassid='".$rown["smallclassid"]."'");
				$rows = fetch_array($rss);
				$nums = $rows['total'];
