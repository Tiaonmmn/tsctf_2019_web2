...
$rs=query("select config from caicaicms_usergroup where groupid=$groupid");
$row=fetch_array($rs);
...
$siteleft=$siteleft. "<div class='contentleft'>";
$rsleft=query("select bigclassid from caicaicms_main where editor='".$editor."'and bigclassid<>0 group by bigclassid");
$rowleft=num_rows($rsleft);
if ($rowleft){
	while ($rowleft=fetch_array($rsleft)){
		$rsb=query("select classname,classzm from caicaicms_zsclass where classid='".$rowleft["bigclassid"]."'");
		$rowb=num_rows($rsb);
		if ($rowb){
		...
		$rsb=query("select count(id) from caicaicms_main where editor='".$editor."'and bigclassid='".$rowleft["bigclassid"]."'");
		...
		$rsn=query("select smallclasszm from caicaicms_main where editor='".$editor."'and bigclasszm='".$rowleft["bigclasszm"]."' and smallclasszm<>''");
		$rown=num_rows($rsn);
		if ($rown){
		$smallclasszms="";
		...
		$rsn=query("select classzm from caicaicms_zsclass where parentid='".$rowleft["bigclasszm"]."' and classzm in ($smallclasszms)");
		$rown=num_rows($rsn);
		if ($rown){
			while ($rown=fetch_array($rsn)){
				$rss=query("select classname from caicaicms_zsclass where classzm='".$rown["classzm"]."'");
				$rows=num_rows($rss);
				if ($rows){
				...
				$rss=query("select count(id) from caicaicms_main where editor='".$editor."'and smallclasszm='".$rown["classzm"]."'");
