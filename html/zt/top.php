...
$rs=query("select * from caicaicms_userdomain where domain='".$_SERVER['HTTP_HOST']."' and passed=1 and del=0");//从顶级级域名中获取用户名
$row=num_rows($rs);
if (!$row){
	$row=fetch_array($rs);
	$editor=$row["username"];
...
$sql="select * from caicaicms_user where username='".$editorinzsshow."'";	//当两都为空时从zsshow接收值
}else{
showmsg ("参数不足!");
}
$rs=query($sql);
...
$rs=query("select skin,skin_mobile,tongji,baidu_map from caicaicms_usersetting where username='".$editor."'");
$row=num_rows($rs);
if ($row){
$row=fetch_array($rs);
$skin=$row["skin"];
...
//showusergroup
$rs=query("select groupname,grouppic,groupid,config from caicaicms_usergroup where groupid=$groupid");
$row=fetch_array($rs);
...
$showad_inzt=str_is_inarr($row["config"],'showad_inzt');//用于判断是否在展厅内显广告
	...
	$rsviptime=query("select startdate from caicaicms_user where username='".$editor."'");
...
//showbanner
$rs=query("select * from caicaicms_usersetting where username='".$editor."'");
$row=num_rows($rs);
if(!$row){
query("INSERT INTO caicaicms_usersetting (username,skin,daohang)VALUES('".$editor."','tongyong','网站首页, 招商信息, 公司简介, 资质证书, 联系方式, 在线留言')");
