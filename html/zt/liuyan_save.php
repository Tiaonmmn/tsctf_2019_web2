...
$rs=query("select * from caicaicms_guestbook where linkmen='$name' and phone='$tel' and saver='$saver'");
$row=num_rows($rs);
if ($row){
showmsg('您已留过言了！');
}else{	
$addok=query("insert into caicaicms_guestbook (content,linkmen,phone,email,saver,sendtime)values('$contents','$name','$tel','$email','$saver','".date('Y-m-d H:i:s')."')");
