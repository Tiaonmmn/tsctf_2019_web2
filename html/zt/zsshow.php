...
$rs=query("select * from caicaicms_main where id='$cpid'");
$row=num_rows($rs);
if(!$row){
showmsg('无记录');
}else{
...
$smallclassname='';
$rsn=query("select classname,classzm from caicaicms_zsclass where classid='".$bigclassid."'");
$rown=fetch_array($rsn);
$bigclassname=$rown["classname"];
$bigclasszm=$rown["classzm"];

if ($smallclassid<>0){
$rsn=query("select classname,classzm from caicaicms_zsclass where classid='".$smallclassid."'");
$rown=fetch_array($rsn);
...
//cp		 
if(showdlinzs=="Yes"){
$rsn=query("select id from caicaicms_dl where cpid='".$cpid."' and passed=1");
$rown=num_rows($rsn);
$liuyan="<tr>";
