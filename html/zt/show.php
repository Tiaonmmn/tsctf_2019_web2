...
$rs=query("select * from caicaicms_main where editor='$editor' and passed=1 order by xuhao desc limit 0,12");
$row=num_rows($rs);
if ($row){
$i=0;
$zslist2='';
...
$sql="select * from caicaicms_pp where editor='".$editor."' and passed=1 order by id desc limit 0,10";
$rs = query($sql); 
...
$sql="select * from caicaicms_zx where editor='".$editor."'and bigclassname='公司新闻' and passed=1 order by id desc limit 0,10";
$rs = query($sql); 
$row= num_rows($rs);  
if(!$row){
...
$sql="select count(*) as total from caicaicms_dl where saver='".$editor."' and del=0";
$rs = query($sql); 
$row = fetch_array($rs);
$totlenum = $row['total'];
if ($totlenum){
$rs=query("select cp,id,cpid,dlsname,sendtime,city,looked from caicaicms_dl where saver='".$editor."' and del=0 order by id desc limit 0,10");
...
$rs=query("select title,content,linkmen,phone,email,looked,sendtime from caicaicms_guestbook where saver='".$editor."'and passed=1 order by id desc limit 0,10");
$row=num_rows($rs);
if ($row){
$list2='';
while ($row=fetch_array($rs)){
...
$rsn=query("select * from caicaicms_user where username='".trim($_COOKIE["UserName"])."'");
