...
<?php
$sql = "select * from caicaicms_userclass where parentid<>'0' order by xuhao asc";
$rs=query($sql);
?>
...
function changelocation(locationid){
	...
	while($row = fetch_array($rs)){
	?>
    <option value="<?php echo $row["classid"]?>" <?php if ($row["classid"]==@$_COOKIE['companyb']){ echo 'selected';}?>><?php echo $row["classname"]?></option>
        <?php
				}
				?>
  </select>
...
$sql="select * from caicaicms_userclass where parentid='" .$_COOKIE['companyb']."'  order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
