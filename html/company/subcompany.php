...
$n=1;
$str='';
$rs=query("select classid,classname from caicaicms_userclass where parentid=0 order by xuhao asc");
$row=num_rows($rs);
if ($row){
...
$n=1;
if ($num<>""){
$sql="select classname,classid from caicaicms_userclass where parentid='". $b ."' order by xuhao limit 0,$num";
}else{
$sql="select classname,classid from caicaicms_userclass where parentid='". $b ."' order by xuhao";
}
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
