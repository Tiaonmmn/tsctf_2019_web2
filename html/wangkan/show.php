...
//在一条记录中上传多张图片建网刊
$sql="select id,title,img,content from caicaicms_wangkan where id='$id'";
$rs = query($sql); 
$row= fetch_array($rs);
if (!$row){
showmsg('不存在相关信息！');
}else{
query("update caicaicms_wangkan set hit=hit+1 where id='$id'");
...
//在小类下用多条记录建网刊
/*$str="";
$sql="select id,title,img,content from caicaicms_zx where passed=1 and bigclassid=14  and smallclassid=17";
