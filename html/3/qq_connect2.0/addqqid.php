...
WriteErrMsg($errmsg);
	...
	$rs=query("select qqid from caicaicms_user where qqid='".$qqid."'");
	$row=num_rows($rs);
	if (!$row){
?>
	...
	<?php
		if ($_POST["isuser"]==1 && $_POST["action"]=="step2") {
				//绑定已有会员帐号
				$sql="select * from caicaicms_user where lockuser=0 and username='". $username ."' and password='". md5($pwd) ."'";
				$rs=query($sql);
				$row=num_rows($rs);
				if (!$row){
				WriteErrMsg("用户名或密码不正确或者您的用户被锁定了。");
				}else{
				query("update caicaicms_user set qqid='".$qqid."' where username='".$username."'");
				//登录
				query("UPDATE caicaicms_user SET showlogintime = lastlogintime where qqid='".$qqid."'");//更新上次登录时间
				query("UPDATE caicaicms_user SET showloginip = loginip where qqid='".$qqid."'");//更新上次登录IP
				query("UPDATE caicaicms_user SET logins = logins+1 where qqid='".$qqid."'");
				query("UPDATE caicaicms_user SET loginip = '".getip()."' where qqid='".$qqid."'");//更新最后登录IP
					if (strtotime(date("Y-m-d H:i:s"))-strtotime($row['lastlogintime'])>86400){
					query("UPDATE caicaicms_user SET totleRMB = totleRMB+".jf_login." WHERE qqid='".$qqid."'");//登录时加积分
					}
				query("UPDATE caicaicms_user SET lastlogintime = '".date('Y-m-d H:i:s')."' WHERE qqid='".$qqid."'");//更新最后登录时间

		
				$rs=query("select username,password from caicaicms_user where qqid='".$qqid."'");
				$row=fetch_array($rs);
				if ($CookieDate==1){
...
$rs=query("select username,password,passwordtrue from caicaicms_user where qqid='".$qqid."'");
$row=fetch_array($rs);
if ($CookieDate==1){
setcookie("UserName",$row['username'],time()+3600*24,"/");
setcookie("PassWord",$row['password'],time()+3600*24,"/");
