...
$n=1;
$str='';
$sql="select classid,classname from caicaicms_specialclass where isshow=1 and parentid=0 order by xuhao asc";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
...
if ($b<>""){
$n=1;
$sql="select classid,classname from caicaicms_specialclass where parentid='".$b."' order by xuhao";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
...
$str="";
$sql="select content,id,title,img from caicaicms_special where bigclassid=$b ";
if ($s!=0){
