...
$sql="select * from caicaicms_special where id='$zxid'";
$rs=query($sql);
$row=fetch_array($rs);
if (!$row){
showmsg('不存在相关信息！');
}else{
query("update caicaicms_special set hit=hit+1 where id='$zxid'");
...
$smallclassname='';
$rs=query("select classname from caicaicms_specialclass where classid='".$bigclassid."'");
$row=fetch_array($rs);
$bigclassname=$row["classname"];

if ($smallclassid<>""){
$rs=query("select classname from caicaicms_specialclass where classid='".$smallclassid."'");
$row=fetch_array($rs);
...
$sql="select * from caicaicms_special where id < ".$zxid." and passed=1 and bigclassid in (select classid from caicaicms_zxclass where isshow=1 and parentid=0) order by id desc limit 0,1";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
...
$sql="select * from caicaicms_special where id > ".$zxid." and passed=1 and bigclassid in (select classid from caicaicms_zxclass where isshow=1 and parentid=0) order by id asc limit 0,1";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
