...
$smallclassname="";
if ($b<>0){
$sql="select * from caicaicms_specialclass where classid='$b'";
$rs=query($sql);
$row=fetch_array($rs);
$bigclassname=$row["classname"];
}
if ($s<>0) {
$sql="select * from caicaicms_specialclass where classid='$s'";
$rs=query($sql);
$row=fetch_array($rs);
...
function formbigclass(){
		$str="";
        $sql = "select * from caicaicms_specialclass where parentid=0";
        $rs=query($sql);
		$row=num_rows($rs);
		if (!$row){
		$str= "请先添加类别名称。";
		...
		if ($b<>0){
		$str="";
        $sql="select * from caicaicms_specialclass where parentid='".$b."' order by xuhao asc";
        $rs=query($sql);
		$row=num_rows($rs);
		if ($row){
			while($row=fetch_array($rs)){
...
$sql="select count(*) as total from caicaicms_special where passed<>0 ";
$sql2='';
if ($b<>0){
...
$sql="select * from caicaicms_special where passed=1 and elite=0";
$sql=$sql.$sql2;
