...
checkid($id,1);
$sqlzs="select * from caicaicms_pp where id='$id'";
$rszs=query($sqlzs);
...
<form action="?do=save" method="post" name="myform" id="myform" onSubmit="return CheckForm();">
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    ...
    <tr> 
      ...
      <td class="border"> 
        <?php
$sql = "select classid,parentid,classname from caicaicms_zsclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
...
function changelocation(locationid){
    ...
    for (i=0;i < onecount; i++){
          ...
          <?php
	$sql = "select classid,classname from caicaicms_zsclass where  parentid=0 order by xuhao asc";
    $rs=query($sql);
	while($row = fetch_array($rs)){
	?>
    <option value="<?php echo $row["classid"]?>" <?php if ($row["classid"]==$rowzs["bigclassid"]) { echo "selected";}?>><?php echo $row["classname"]?></option>
          ...
          <?php
$sql="select classid,classname from caicaicms_zsclass where parentid='" .$rowzs["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
...
query("update caicaicms_pp set bigclassid='$bigclassid',smallclassid='$smallclassid',ppname='$cpname',sm='$sm',img='$img',sendtime='$sendtime',passed='$passed' where id='$cpid'");
...
query("update caicaicms_pp set editor='$editor',userid='$userid',comane='$comane',passed='$passed' where id='$cpid'");
}
