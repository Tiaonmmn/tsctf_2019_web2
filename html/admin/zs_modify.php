...
$sql="select * from caicaicms_main where id='$id'";
$rs=query($sql);
$row=fetch_array($rs);
?>
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    ...
    <tr> 
      ...
      <td class="border"> 
		...
		$rowB= num_rows($rsB);
		if(!$rowB){
		echo "<a href='class2.php?tablename=caicaicms_zsclass'>添加大类</a>";
		}else{
...
$isok=query("update caicaicms_main set bigclassid='$bigclassid',smallclassid='$smallclassid',smallclassids='$smallclassids',szm='$szm',prouse='$prouse',proname='$cpname',sm='$sm',img='$img',
...
if ($editor<>$oldeditor) {
$rs=query("select groupid,qq,comane,id,renzheng,province,city,xiancheng from caicaicms_user where username='".$editor."'");
$row = num_rows($rs);
if ($row){
$row = fetch_array($rs);
$groupid=$row["groupid"];
...
query("update caicaicms_main set editor='$editor',userid='$userid',groupid='$groupid',qq='$qq',comane='$comane',province_user='$province_user',city_user='$city_user',xiancheng_user='$xiancheng_user',renzheng='$renzheng' where id='$cpid'");
}
