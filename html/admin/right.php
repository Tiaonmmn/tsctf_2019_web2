<?php
include ("admin.php");
?>
<!DOCTYPE html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title>后前管理首页</title>
<link href="style.css" rel="stylesheet" type="text/css">
</head>

<body>
<div class="border" style="font-size:14px;line-height:25px"> 
<?php
echo  "请先看看下面是否有需要您处理的工作。<br>";			
function checkisendsever()
{
//到期的招商产品取消推荐
query("update caicaicms_main set elite=0 where eliteendtime< '".date('Y-m-d H:i:s')."'");
//检查到期的vip用户
$sql="select groupid,enddate,username from caicaicms_user where groupid>1 and enddate<'".date('Y-m-d H:i:s')."'";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
while($row=fetch_array($rs)){
		query("update caicaicms_user set groupid=1 where username='".$row["username"]."'");
		query("Update caicaicms_main set groupid=1 where editor='" . $row["username"] . "'");
		query("Update caicaicms_main set elite=0 where editor='" . $row["username"] . "'");
}
}
echo "已完成";
}

$sql="select id from caicaicms_usermessage where reply is null";
$rs=query($sql);
$row=num_rows($rs);
echo "用户返馈信息 ".$row." 条 [ <a href='usermessage.php'>查看</a> ]<br>";

$sql="select id from caicaicms_user where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审注册用户 ".$row." 条 [ <a href='usermanage.php?shenhe=no'>查看</a> ]<br>";

$sql="select id from caicaicms_userdomain where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审域名绑定 ".$row." 条 [ <a href='domain_manage.php?shenhe=no'>查看</a> ]<br>";

$sql="select id from caicaicms_main where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审".channelzs."信息 ".$row." 条 [ <a href='zs_manage.php?shenhe=no'>查看</a> ]<br>";


if (str_is_inarr(channel,'pp')=='yes'){
$sql="select id from caicaicms_pp where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审品牌信息 ".$row." 条 [ <a href='pp_manage.php?shenhe=no'>查看</a> ]<br>";
}

$sql="select id from caicaicms_dl where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审".channeldl."信息 ".$row." 条 [ <a href='dl_manage.php?shenhe=no'>查看</a> ]<br>";

$sql="select id from caicaicms_guestbook where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审展厅留言 ".$row." 条 [ <a href='ztliuyan_manage.php?shenhe=no'>查看</a> ]<br>";

if (str_is_inarr(channel,'zh')=='yes'){
$sql="select id from caicaicms_zh where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审展会信息 ".$row." 条 [ <a href='zh_manage.php?shenhe=no'>查看</a> ]<br>";
}

if (str_is_inarr(channel,'zx')=='yes'){
$sql="select id from caicaicms_zx where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审资讯信息 ".$row." 条 [ <a href='zx_manage.php?shenhe=no'>查看</a> ]<br>";
}

if (str_is_inarr(channel,'wangkan')=='yes'){
$sql="select id from caicaicms_wangkan where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审网刊信息 ".$row." 条 [ <a href='wangkan_manage.php?shenhe=no'>查看</a> ]<br>";
}

if (str_is_inarr(channel,'baojia')=='yes'){
$sql="select id from caicaicms_baojia where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审报价信息 ".$row." 条 [ <a href='baojia_manage.php?shenhe=no'>查看</a> ]<br>";
}

if (str_is_inarr(channel,'special')=='yes'){
$sql="select id from caicaicms_special where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审专题信息 ".$row." 条 [ <a href='special_manage.php?shenhe=no'>查看</a> ]<br>";
}

if (str_is_inarr(channel,'job')=='yes'){
$sql="select id from caicaicms_job where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审招聘信息 ".$row." 条 [ <a href='job_manage.php?shenhe=no'>查看</a> ]<br>";
}

if (str_is_inarr(channel,'ask')=='yes'){
$sql="select id from caicaicms_ask where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审问答信息 ".$row." 条 [ <a href='ask_manage.php?shenhe=no'>查看</a> ]<br>";
}

if (str_is_inarr(channel,'zx')=='yes'){
$sql="select id from caicaicms_pinglun where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审评论信息 ".$row." 条 [ <a href='pinglun_manage.php?shenhe=no'>查看</a> ]<br>";
}

$sql="select id from caicaicms_link where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审友情链接 ".$row." 个 [ <a href='linkmanage.php?shenhe=no'>查看</a> ]<br>";

$sql="select id from caicaicms_textadv where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审文字广告 ".$row." 个 [ <a href='ad_user_manage.php?shenhe=no'>查看</a> ]<br>";

$sql="select id from caicaicms_licence where passed=0";
$rs=query($sql);
$row=num_rows($rs);
echo "待审资质证书 ".$row." 个 [ <a href='licence.php?shenhe=no'>查看</a> ]<br>";

$sql="select id from caicaicms_ad where endtime< '".date('Y-m-d')."'";
$rs=query($sql);
$row=num_rows($rs);
echo "已到期的广告 ".$row." 条 [ <a href='ad_manage.php?action=showendtime'>查看</a> ]<br>";

$sql="select id from caicaicms_usermessage where reply is null";
$rs=query($sql);
$row=num_rows($rs);
echo "未回复的反馈 ".$row." 条 [ <a href='usermessage.php?reply=no'>查看</a> ]<br>";

$sql="select id from caicaicms_bad";
$rs=query($sql);
$row=num_rows($rs);
echo "不良操作记录 ".$row." 条 [ <a href='showbad.php'>查看</a> ]<br>";
?>
</div>
<div class="border">
<?php 
if (@$action=="check") {
checkisendsever();
}else{
?>
<a href="?action=check">检查处理到期的注册用户，到期的<?php echo channelzs?>产品取消推荐</a>
<?php
}
?>	 
</div>  
</body>
</html>