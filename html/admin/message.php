...
if ($action=="add"){
query("INSERT INTO caicaicms_message (sendto,title,content,sendtime)VALUES('$sendto','$title','$content','".date('Y-m-d H:i:s')."')");
}elseif ($action=="modify") {
checkid($id);
query("update caicaicms_message set sendto='$sendto',title='$title',content='$content' where id='$id'");
}
...
checkid($id);
if (@$action=="del"){
query("delete from caicaicms_message where id='$id'");
...
<?php
$sql="select * from caicaicms_message order by id desc";
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
echo "暂无信息";
...
checkid($id,1);
$sql="select * from caicaicms_message where id='$id'";
$rs=query($sql);
$row=fetch_array($rs);
?>
