...
if ($img<>''){
	if (substr($img,0,4) == "http"){
		...
		$img=substr($img_bendi,strpos($img_bendi,"/uploadfiles"));//注：这里变量要换成$img以供入库用。在grabimg函数中$img被加了caicaicmsroot这里要去掉
	}
...
$isok=query("INSERT INTO caicaicms_ad (bigclassname,smallclassname,title,titlecolor,link,img,username,starttime,endtime,elite,sendtime)
VALUES
...
$isok=query("update caicaicms_ad set bigclassname='$bigclassname',smallclassname='$smallclassname',title='$title',titlecolor='$titlecolor',link='$link',
...
<?php
$sql = "select classname,parentid from caicaicms_adclass where parentid<>'A' order by xuhao asc";
$rs=query($sql);
?>
...
function changelocation(locationid){
	...
	<select name="bigclassid" onChange="changelocation(document.myform.bigclassid.options[document.myform.bigclassid.selectedIndex].value)" size="1">
          ...
          <?php
	$sql = "select classname from caicaicms_adclass where parentid='A' order by xuhao asc";
    $rs=query($sql);
	while($row = fetch_array($rs)){
	?>
...
$sql="select classname from caicaicms_adclass where parentid='" .$_COOKIE["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
	?>
...
$sql="select * from caicaicms_ad where id='$id'";
$rs=query($sql);
$row=fetch_array($rs);
?>
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      ...
      <td width="81%" class="border"> 
       <?php
$sqln = "select classname,parentid from caicaicms_adclass where parentid<>'A' order by xuhao asc";
$rsn=query($sqln);
?>
        <script language = "JavaScript" type="text/JavaScript">
var onecount;
...
function changelocation(locationid){
    ...
    for (i=0;i < onecount; i++){
          ...
          <option value="" selected="selected">请选择大类别</option>
          <?php
	$sqln = "select classname from caicaicms_adclass where  parentid='A' order by xuhao asc";
    $rsn=query($sqln);
	while($rown = fetch_array($rsn)){
	?>
<option value="<?php echo $rown["classname"]?>" <?php if ($rown["classname"]==$row["bigclassname"]) { echo "selected";}?>><?php echo $rown["classname"]?></option>
          ...
          <option value="">不指定小类</option>
          <?php
$sqln="select classname from caicaicms_adclass where parentid='" .$row["bigclassname"]."' order by xuhao asc";
$rsn=query($sqln);
while($rown = fetch_array($rsn)){
	?>
