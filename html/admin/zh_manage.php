...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	checkid($id);
	$sql="select passed from caicaicms_zh where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_zh set passed=1 where id ='$id'");
		}else{
		query("update caicaicms_zh set passed=0 where id ='$id'");
		}
	}
}else{
...
<input type="submit" class="buttons" onClick="javascript:location.href='zh.php?do=add'" value="发布展会信息">			  
    ...
    <div class="border2"> 
      <?php	
$str=""	;
$sql="select classid,classname from caicaicms_zhclass order by xuhao";
$rs = query($sql); 
...
$sql="select count(*) as total from caicaicms_zh where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_zh where id<>0 ";
$sql=$sql.$sql2;
...
?>
    <tr class="trcontent"> 
	  ...
	  $rsn=query("select classname from caicaicms_zhclass where classid='".$row["bigclassid"]."'");
...
?>
  ...
  <div class="border"> <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
      ...
      <input name="tablename" type="hidden"  value="caicaicms_zh">
  </div>
</form>
