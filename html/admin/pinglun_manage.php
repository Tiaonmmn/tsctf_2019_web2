...
if(!empty($_POST['id'])){
	...
	switch ($action){
	case "pass";
	$sql="select passed from caicaicms_pinglun where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_pinglun set passed=1 where id ='$id'");
		}else{
		query("update caicaicms_pinglun set passed=0 where id ='$id'");
		}
	break;	
	case "del";
	query("delete from caicaicms_pinglun where id ='$id'");
	break;	
	}	
	}
}else{
...
$sql="select count(*) as total from caicaicms_pinglun where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_pinglun where id<>0 ";
$sql=$sql.$sql2;
...
?>
      ...
      <div class="border"> <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
        ...
        <input type="submit" onClick="myform.action='del.php';myform.target='_self';return ConfirmDel()" value="【删除】选中的信息"> 
		 <input name="tablename" type="hidden"  value="caicaicms_pinglun">
