...
foreach ($imgs as $value) {
	...
	$img_bendi=substr($img_bendi,strpos($img_bendi,"/uploadfiles"));//在grabimg函数中$img被加了caicaicmsroo。这里要去掉
...
//}
$isok=query("insert into caicaicms_ask(bigclassid,smallclassid,title,content,img,editor,elite,passed,sendtime) values('$bigclassid','$smallclassid','$title','$content','$img','管理员','$elite','$passed','".date('Y-m-d H:i:s')."')");
$id=insert_id();	

}elseif ($_REQUEST["action"]=="modify"){
checkadminisdo("ask_modify");
$isok=query("update caicaicms_ask set bigclassid='$bigclassid',smallclassid='$smallclassid',title='$title',content='$content',img='$img',
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      ...
      <td width="88%" class="border" > 
        <?php
$sql = "select * from caicaicms_askclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
...
$sql="select * from caicaicms_askclass where parentid=" .$_COOKIE["askbigclassid"]." order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
...
$rszx = query("select * from caicaicms_ask where id='$id'");
...
<form action="ask.php?do=save" method="post" name="myform" id="myform" onSubmit="return CheckForm();">
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="5">
    <tr> 
      ...
      <td width="88%" class="border"> 
        <?php
$sql = "select parentid,classid,classname from caicaicms_askclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
        <script language = "JavaScript" type="text/JavaScript">
var onecount;
...
function changelocation(locationid){
    ...
    if (subcat[i][1] == locationid){ document.myform.smallclassid.options[document.myform.smallclassid.length] = new Option(subcat[i][0], subcat[i][2]);}        
          ...
          <?php
	$sql = "select classid,classname from caicaicms_askclass where  parentid=0 order by xuhao asc";
    $rs=query($sql);
	while($row = fetch_array($rs)){
	?>
<option value="<?php echo $row["classid"]?>" <?php if ($row["classid"]==$rowzx["bigclassid"]) { echo "selected";}?>><?php echo $row["classname"]?></option>
          ...
          <?php
$sql="select classid,classname from caicaicms_askclass where parentid='" .$rowzx["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
	?>
