...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	checkid($id);
	$sql="select elite from caicaicms_help where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['elite']=='0'){
		query("update caicaicms_help set elite=1 where id ='$id'");
		}else{
		query("update caicaicms_help set elite=0 where id ='$id'");
		}
	}
}else{
...
$sql="select * from caicaicms_help where classid='".$b."' ";
if ($keyword<>"") {  		
...
?>
  ...
  <div class="border"> <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
	...
	<input name="tablename" type="hidden"  value="caicaicms_help">
