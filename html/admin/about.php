...
if ($action=="save" ){
	...
	query("insert into caicaicms_about (title,content)VALUES('$title','$content') ");
	$go=1;
	//echo "<script>location.href='about_manage.php'<//script>";	
	}elseif ($saveas=="modify"){
	checkadminisdo("about_modify");
	query("update caicaicms_about set title='$title',content='$content',link='$link' where id='". $id."' ");
	$go=1;
	//echo "<script>location.href='about_manage.php'<//script>";
	}
}
...
if ($action=="modify") {
//checkadminisdo("about_modify");
$sql="select * from caicaicms_about where id='".$id."'";
$rs=query($sql);
$row=fetch_array($rs);
?>
