...
if ($action=="add"){	
	$sql="Select * From caicaicms_admingroup where groupname='".$groupname."'";
	$rs = query($sql);
	...
	$sql="insert into caicaicms_admingroup (groupname,config)values('$groupname','$config')";
	query($sql);
	echo "<script>location.href='?'</script>";
	}
}elseif($action=="modify"){
$sql="update caicaicms_admingroup set groupname='$groupname',config='$config' where id='$id'";
$isok=query($sql);
...
$id=trim($_GET["id"]);
if  ($groupname<>""){
	$sql="delete from caicaicms_admingroup where groupname='" . $groupname . "'";
	query($sql);
} 
echo  "<script>location.href='?'</script>";
}

$sql="select * from caicaicms_admingroup";
$rs = query($sql); 
?>
...
$sql="select * from caicaicms_admingroup where id='$id'";
$rs = query($sql); 
$row= num_rows($rs);
if (!$row){
echo "暂无信息";
