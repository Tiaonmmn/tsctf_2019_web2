...
if(!empty($_POST['id'])){
	...
	switch ($action){
	case "pass";
	$sql="select passed from caicaicms_link where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_link set passed=1 where id ='$id'");
		}else{
		query("update caicaicms_link set passed=0 where id ='$id'");
		}
	break;	
	case "elite";
	$sql="select elite from caicaicms_link where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['elite']=='0'){
		query("update caicaicms_link set elite=1 where id ='$id'");
		}else{
		query("update caicaicms_link set elite=0 where id ='$id'");
		}
	break;
	//case "del";
	//query("delete from caicaicms_link where id ='$id'");
	//break;	
	}	
	}
}else{
...
$sql="select count(*) as total from caicaicms_link where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_link where id<>0 ";
$sql=$sql.$sql2;
...
?>
	<tr class="trcontent">  
	  ...
	  <?php
	  $rsn=query("select classname from caicaicms_linkclass where classid=".$row["bigclassid"]." ");
...
?>
      ...
      <input name="tablename" type="hidden"  value="caicaicms_link">
