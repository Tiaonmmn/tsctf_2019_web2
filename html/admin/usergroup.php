...
if  ($groupid<>"") {
	query("delete from caicaicms_usergroup where groupid='$groupid'");
}
...
<?php
$sql="select * from caicaicms_usergroup";
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
echo "暂无信息";
...
checkadminisdo("usergroup");
	...
	$sql="Select * From caicaicms_usergroup Where groupid='".$groupid."'";
	$rs=query($sql);
	$row=num_rows($rs);
	if ($row){
		$FoundErr=1;
	...
	$sql="select * from caicaicms_usergroup where groupname='" . $groupname . "'";
	$rs=query($sql);
	$row=num_rows($rs);
	if ($row){
		$FoundErr=1;
	...
	WriteErrMsg($ErrMsg);
		...
		query("insert into caicaicms_usergroup (
	...
	checkadminisdo("usergroup");
	checkid($id);
	query("update caicaicms_usergroup set groupname='$groupname',grouppic='$grouppic',groupid='$groupid',RMB='$RMB',config='$config',
	...
	query("Update caicaicms_user set groupid='" . $groupid . "' where groupid='" . $oldgroupid."'");
	query("Update caicaicms_main set groupid='" . $groupid . "' where groupid='" . $oldgroupid."'");
    }		
	echo "<script>location.href='?'</script>";
}
}
...
$sql="Select * from caicaicms_usergroup where id='$id'";
$rs=query($sql);
$row=fetch_array($rs);
?>
