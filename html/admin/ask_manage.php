...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	$sql="select passed from caicaicms_ask where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_ask set passed=1,sendtime='".date('Y-m-d H:i:s')."' where id ='$id'");
		}else{
		query("update caicaicms_ask set passed=0,sendtime='".date('Y-m-d H:i:s')."' where id ='$id'");
		}
	}
}else{
...
<div class="border2">
    <?php	
$str="";	
$sql="select classid,classname from caicaicms_askclass where parentid=0 order by xuhao";
$rs = query($sql); 
...
$sql="select count(*) as total from caicaicms_ask where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_ask where id<>0 ";
$sql=$sql.$sql2;
...
?>
      ...
      <div class="border"> <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
        ...
        <input name="tablename" type="hidden"  value="caicaicms_ask"></div>
