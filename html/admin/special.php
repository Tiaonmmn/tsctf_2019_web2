...
foreach ($imgs as $value) {
	...
	$img_bendi=substr($img_bendi,strpos($img_bendi,"/uploadfiles"));//在grabimg函数中$img被加了caicaicmsroo这里要去掉
...
if ($img<>''){
	if (substr($img,0,4) == "http"){//$img=trim($_POST["img"])的情况下，这里有可能是远程图片地址
		...
		$img=substr($img,strpos($img,"/uploadfiles"));//在grabimg函数中$img被加了caicaicmsroo。这里要去掉
	}
		
	$imgsmall=str_replace(siteurl,"",getsmallimg($img));
	if (file_exists(caicaicmsroot.$imgsmall)===false && file_exists(caicaicmsroot.$img)!==false){//小图不存在，且大图存在的情况下，生成缩略图
	makesmallimg($img);//同grabimg一样，函数里加了caicaicmsroot
	}	
}
//---end
...
//$sql="select title,editor from caicaicms_special where title='".$title."'";
//$rs = query($sql); 
//$row= fetch_array($rs);
//if ($row){
...
$isok=query("insert into caicaicms_special
...
$isok=query("update caicaicms_special set bigclassid='$bigclassid',bigclassname='$bigclassname',smallclassid='$smallclassid',smallclassname='$smallclassname',title='$title',laiyuan='$laiyuan',
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      ...
      <td width="85%" class="border" > 
        <?php
$sql = "select * from caicaicms_specialclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
...
$sql="select * from caicaicms_specialclass where parentid='" .$_COOKIE["specialbigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
...
(0-127之间的数字，数值大的排在前面) </td>
    ...
    <tr> 
      ...
      <td class="border" ><select name="groupid">
          ...
          <?php
		  $rs=query("Select * from caicaicms_usergroup ");
		  while($row = fetch_array($rs)){
...
$rszx = query("select * from caicaicms_special where id='$id'");
...
<form action="?do=save" method="post" name="myform" id="myform" onSubmit="return CheckForm();">     
  <table width="100%" border="0" cellspacing="0" cellpadding="5">
    <tr> 
      ...
      <td width="85%" class="border"> 
        <?php
$sql = "select classid,parentid,classname from caicaicms_specialclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
        <script language = "JavaScript" type="text/JavaScript">
var onecount;
...
<option value="0">不指定小类</option>
<?php
$sql="select classid,classname from caicaicms_specialclass where parentid='" .$rowzx["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
...
?>
    ...
    <tr> 
      ...
      <td class="border" > <select name="groupid">
          <option value="0">全部用户</option>
          <?php
		  $rs=query("Select groupid,groupname from caicaicms_usergroup ");
		  while($row = fetch_array($rs)){
