...
if ($tablename=="caicaicms_main"){
checkadminisdo("zs_del");
if (strpos($id,",")>0){
$sql="select img,flv,id,sm from caicaicms_main where id in (". $id .")";
}else{
$sql="select img,flv,id,sm from caicaicms_main where id='$id'";
}
$rs=query($sql);
while($row=fetch_array($rs)){
	...
	foreach ($imgs as $value) {
		...
		query("delete from caicaicms_main where id=".$row['id']."");
		query("update caicaicms_dl set cpid=0 where cpid=".$row["id"]."");//把代理信息中的ID设为0
}
echo "<script>location.href='".$pagename."'</script>"; 

}elseif ($tablename=="caicaicms_pp" || $tablename=="caicaicms_licence"|| $tablename=="caicaicms_ad"){
...
}elseif ($tablename=="caicaicms_zx" || $tablename=="caicaicms_special" || $tablename=="caicaicms_ask"){
...
}elseif ($tablename=="caicaicms_dl"){checkadminisdo("dl_del");del();
}elseif ($tablename=="caicaicms_zh"){checkadminisdo("zh_del");del();
}elseif ($tablename=="caicaicms_job"){checkadminisdo("job_del");del();
}elseif ($tablename=="caicaicms_baojia"){checkadminisdo("baojia_del");del();
}elseif ($tablename=="caicaicms_link"){checkadminisdo("friendlink");del();
}else{del();}
