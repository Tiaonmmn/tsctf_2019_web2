...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	checkid($id);
	$sql="select passed from caicaicms_user where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_user set passed=1 where id ='$id'");
		}else{
		query("update caicaicms_user set passed=0 where id ='$id'");
		}
	}
}else{
...
$sql="select count(*) as total from caicaicms_user where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_user where id<>0 ";
$sql=$sql.$sql2;
...
<?php
	$rsn=query("select config from caicaicms_admingroup where id=(select groupid from caicaicms_admin where pass='".@$_COOKIE["pass"]."' and admin='".@$_COOKIE["admin"]."')");//只验证密码会出现，两个管理员密码相同的情况，导致出错,前加@防止_COOKIE失效后出错提示
...
?>
	...
	?>
	  ...
	  $rskind=query("select classname from caicaicms_userclass where classid=".$row["bigclassid"]."");
      ...
      <td align="center"> <?php
	$rsn=query("select groupname from caicaicms_usergroup where groupid='".$row["groupid"]."'");
	$rown=fetch_array($rsn);
