...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	$sql="select passed from caicaicms_wangkan where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_wangkan set passed=1 where id ='$id'");
		}else{
		query("update caicaicms_wangkan set passed=0 where id ='$id'");
		}
	}
}else{
...
<?php
$str="";		
$sql="select classid,classname from caicaicms_wangkanclass order by xuhao";
$rs = query($sql); 
...
$sql="select count(*) as total from caicaicms_wangkan where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_wangkan where id<>0 ";
$sql=$sql.$sql2;
...
?>
    <tr class="trcontent"> 
	  ...
	  $rsn=query("select classname from caicaicms_wangkanclass where classid='".$row["bigclassid"]."'");
...
?>
      ...
      <div class="border"> <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
        ...
        <input name="tablename" type="hidden"  value="caicaicms_wangkan">
      </div>
</form>
