...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	checkid($id);
	$sql="select passed from caicaicms_main where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_main set passed=1 where id ='$id'");
		}else{
		query("update caicaicms_main set passed=0 where id ='$id'");
		}
	}
}else{
...
$sql="select count(*) as total from caicaicms_main where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_main where id<>0 ";
$sql=$sql.$sql2;
 ...
 ?>
  ...
  <div class="border">
    <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
        ...
        <input name="tablename" type="hidden"  value="caicaicms_main">
      </div>
</form>
