...
<form action="?do=save" method="post" name="myform" id="myform" onSubmit="return CheckForm();">
  <table width="100%" border="0" cellpadding="3" cellspacing="0">
    ...
    <tr> 
      ...
      <td class="border"> 
	   <?php
		$sqln = "select classid,classname from caicaicms_zsclass where parentid=0 order by xuhao asc";
	    $rsn=query($sqln);
        $rown=num_rows($rsn);
		if (!$rown){
			echo "请先添加栏目。";
...
if ($_POST["action"]=="add"){
	if ($cp<>'' && $truename<>'' && $tel<>''){
	$addok=query("Insert into caicaicms_baojia(classid,cp,province,city,xiancheng,price,danwei,companyname,truename,tel,address,email,sendtime) 		values('$classid','$cp','$province','$city','$xiancheng','$price','$danwei','$companyname','$truename','$tel','$address','$email','".date('Y-m-d H:i:s')."')") ;
	}
...
$addok=query("update caicaicms_baojia set classid='$classid',cp='$cp',province='$province',city='$city',xiancheng='$xiancheng',price='$price',danwei='$danwei',companyname='$companyname',truename='$truename',tel='$tel',address='$address',email='$email',sendtime='".date('Y-m-d H:i:s')."',passed='$passed' where id='$id'");
}
if ($addok){
