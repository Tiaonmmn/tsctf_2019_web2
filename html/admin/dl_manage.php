...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	checkid($id);
	$sql="select passed from caicaicms_dl where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
	if ($row['passed']=='0'){
	query("update caicaicms_dl set passed=1 where id ='$id'");
    }else{
	query("update caicaicms_dl set passed=0 where id ='$id'");
	}
	}	
}else{
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="border">
  ...
  <?php	
$str='';
$rs = query("select classid,classname from caicaicms_zsclass where parentid=0 order by xuhao");
if ($rs){//当不出错时
...
$sql="select count(*) as total from caicaicms_dl where id<>0 ";
$rs =query($sql.$sql2); 
$row = fetch_array($rs);
$totlenum = $row['total'];
$totlepage=ceil($totlenum/$page_size);

$sql="select * from caicaicms_dl where id<>0 ";
$sql=$sql.$sql2;
$sql=$sql . " order by id desc limit $offset,$page_size";
//$sql=$sql." and id>=(select id from caicaicms_dl order by id limit $offset,1) order by id desc limit $page_size";
$rs = query($sql); 
if(!$totlenum){
echo "暂无信息";
}else{
...
<form name="myform" id="myform" method="post" action="" onSubmit="return anyCheck(this.form)">
      ...
      <div class="border2"> 
        ...
        <input name="tablename" type="hidden"  value="caicaicms_dl"> </div>
