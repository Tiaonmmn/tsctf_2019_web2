...
checkid($id,1);
$sqlzs="select * from caicaicms_job where id='$id'";
$rszs=query($sqlzs);
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      ...
      <td width="82%" class="border"> 
        <?php
$sql = "select classid,parentid,classname from caicaicms_jobclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
...
<option value="<?php echo $row["classid"]?>" <?php if ($row["classid"]==$rowzs["bigclassid"]) { echo "selected";}?>><?php echo $row["classname"]?></option>
          ...
          <?php
$sql="select classid,classname from caicaicms_jobclass where parentid='" .$rowzs["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
...
query("update caicaicms_job set bigclassid='$bigclassid',bigclassname='$bigclassname',smallclassid='$smallclassid',smallclassname='$smallclassname',jobname='$jobname',sm='$sm',sendtime='$sendtime',passed='$passed' where id='$cpid'");
if ($editor<>$oldeditor) {
$rs=query("select comane,id from caicaicms_user where username='".$editor."'");
$row = num_rows($rs);
if ($row){
$row = fetch_array($rs);
$userid=$row["id"];
...
query("update caicaicms_job set editor='$editor',userid='$userid',comane='$comane' where id='$cpid'");
}
