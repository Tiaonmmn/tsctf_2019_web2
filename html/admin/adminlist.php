...
query("delete from caicaicms_admin where id='".$id."'");
echo  "<script>alert('删除成功');location.href='?'</script>";
}
$sql="select * from caicaicms_admin order by id desc";
$rs = query($sql); 
?>
...
?>
  <tr class="trcontent">
	...
	<?php 
			$rsn=query("select groupname from caicaicms_admingroup where id='".$row['groupid']."'");
			$r=num_rows($rsn);
			if ($r){
			$r=fetch_array($rsn);
			echo $r["groupname"];
    ...
    <td align="center"><a href="?do=modify&admins=<?php echo $row["admin"]?>">修改权限</a> 
	 ...
	 <?php
$rsn2=query("select id from caicaicms_admin where groupid=(select id from caicaicms_admingroup where groupname='超级管理员')");
$rown2=num_rows($rsn2);//超级管理员数	 
	 
$rsn=query("select groupname from caicaicms_admingroup where id=(select groupid from caicaicms_admin where id=".$row["id"].")");
...
if ($action=="add"){
	...
	$sql="select admin from caicaicms_admin where admin='".$admins."'";
	$rs = query($sql);
	...
	if ($founderr==1){
		...
		$sql="insert into caicaicms_admin (admin,pass,groupid) values ('$admins','$passs','$groupid')";
		query($sql);
    ...
    <tr> 
      ...
      <td class="border">
	   <select name="groupid" id="groupid">
          <?php
    $sql="Select * from caicaicms_admingroup order by id asc";
    $rs = query($sql); 
	while($row= fetch_array($rs)){
...
query("update caicaicms_admin set groupid='$groupid' where admin='".$admins."'");
echo "<SCRIPT language=JavaScript>alert('修改成功！');location.href='?'</SCRIPT>";	
}else{
$sql="select * from caicaicms_admin where admin='" . $admins . "'";
$rs = query($sql);
$row= fetch_array($rs);
?>
  ...
  <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
    ...
    <tr> 
      ...
      <td class="border"> <select name="groupid">
          <?php
	$sqln="Select * from caicaicms_admingroup order by id asc";
	$rsn =query($sqln,$conn);
	$rown= num_rows($rsn);
	if ($rown){
...
if ($action=="modify"){
	...
	query("update caicaicms_admin set pass='$passs' where admin='".$admins."'");
