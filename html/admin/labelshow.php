...
if ($channel=='zsshow' || $channel=='ppshow' || $channel=='askshow'|| $channel=='zxshow'||$channel=='jobshow'||$channel=='specialshow'){
	if ($channel=='askshow'){
	$sql = "select parentid,classname,classid from caicaicms_askclass where parentid<>0 order by classid asc";
	}elseif($channel=='zxshow'){
	$sql = "select parentid,classname,classid from caicaicms_zxclass where parentid<>0 order by classid asc";
	}elseif($channel=='specialshow'){
	$sql = "select parentid,classname,classid from caicaicms_specialclass where parentid<>0 order by classid asc";
	}elseif($channel=='jobshow'){
	$sql = "select parentid,classname,classid from caicaicms_jobclass where parentid<>0 order by classid asc";
	}else{
	$sql = "select parentid,classname,classid from caicaicms_zsclass where parentid<>0 order by classid asc";
	}
	$rs=query($sql);
	?>
...
<?php
	if ($channel=='askshow'){
	$sql = "select classname,classid from caicaicms_askclass parentid=0 order by xuhao asc";
	}elseif ($channel=='companyshow'){
	$sql = "select classid,classname from caicaicms_userclass where parentid=0 order by xuhao asc";
	}elseif ($channel=='zxshow'){
	$sql = "select classid,classname from caicaicms_zxclass where parentid=0 order by xuhao asc";
	}elseif ($channel=='specialshow'){
	$sql = "select classid,classname from caicaicms_specialclass where parentid=0 order by xuhao asc";
	}elseif ($channel=='jobshow'){
	$sql = "select classid,classname from caicaicms_jobclass where parentid=0 order by xuhao asc";
	}elseif ($channel=='zhshow'){
	$sql = "select classid,classname from caicaicms_zhclass  order by xuhao asc";
	}elseif ($channel=='wangkanshow'){
	$sql = "select classid,classname from caicaicms_wangkanclass  order by xuhao asc";
	}elseif ($channel=='linkshow'){
	$sql = "select classid,classname from caicaicms_linkclass  order by xuhao asc";
	}else{
	$sql = "select classname,classid from caicaicms_zsclass where parentid=0 order by xuhao asc";
	}	 
    $rs=query($sql);
	while($r=fetch_array($rs)){
			?>
	...
	if ($bigclassid<>0 && $bigclassid<>"empty" && $bigclassid<>""){
		if ($channel=='askshow'){
		$sql="select classid,classname from caicaicms_askclass where parentid='" . $bigclassid ."' order by classid asc";
		}elseif ($channel=='zxshow'){
		$sql="select classid,classname from caicaicms_zxclass where parentid='" . $bigclassid ."' order by classid asc";
		}elseif ($channel=='specialshow'){
		$sql="select classid,classname from caicaicms_specialclass where parentid='" . $bigclassid ."' order by classid asc";
		}elseif ($channel=='jobshow'){
		$sql="select classid,classname from caicaicms_jobclass where parentid='" . $bigclassid ."' order by classid asc";
		}else{
		$sql="select classid,classname from caicaicms_zsclass where parentid='" . $bigclassid ."' order by classid asc";
		}
			$rs=query($sql);
			while($r=fetch_array($rs)){
		...
		<select name="groupid">
		 <option value="0" >所的会员</option>
          <?php
			$rsn=query("select groupid,groupname from caicaicms_usergroup order by groupid asc");
			$r=num_rows($rsn);
			if ($r){
...
只调用<?php channeldl?>留言 </label>
		...
		<select name="id">
          ...
          <?php
       $sql = "select id,title from caicaicms_about order by id desc";
       $rs=query($sql);
		   while($r=fetch_array($rs)){
			?>
