...
$sql="select xuhao,id from caicaicms_ad where bigclassname='$b' and smallclassname='$s'";
$rs = query($sql); 
...
query("update caicaicms_ad set xuhao=$xuhao where id=".$row['id']."");
}
}
?>
...
<?php	
$str="大类：";
$sql="select classname from caicaicms_adclass where parentid='A' order by xuhao";
$rs = query($sql); 
while($row = fetch_array($rs)){
...
if ($b<>''){
$str="<br>小类：";
$sql="select classname from caicaicms_adclass where parentid='".$b."' order by xuhao";
$rs = query($sql); 
while($row = fetch_array($rs)){
...
<?php
$sql="select * from caicaicms_ad where id<>0 ";
if ($b<>"") {  		
