...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	checkid($id);
	$sql="select passed from caicaicms_zx where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_zx set passed=1,sendtime='".date('Y-m-d H:i:s')."' where id ='$id'");
		}else{
		query("update caicaicms_zx set passed=0,sendtime='".date('Y-m-d H:i:s')."' where id ='$id'");
		}
	}
}else{
...
<div class="border2">
    ...
    <?php	
$sql="select classid,classname from caicaicms_zxclass where parentid=0 order by xuhao";
$rs = query($sql); 
...
$sql="select count(*) as total from caicaicms_zx where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_zx where id<>0 ";
$sql=$sql.$sql2;
...
?>
  ...
  <div class="border"> <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
      ...
      <input name="tablename" type="hidden"  value="caicaicms_zx">
  </div>
</form>
