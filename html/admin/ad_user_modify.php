...
$isok=query("update caicaicms_textadv set adv='$adv',advlink='$advlink',img='$img',passed=1 where id='$id'");
$rs=query("select username from caicaicms_textadv where id='$id'");
$row=fetch_array($rs);
$advusername=$row["username"];
if ($advusername !=''){//加此判断以免把所有广告都给改了
query("update caicaicms_ad set title='$adv',link='$advlink',img='$img' where username='".$advusername."'");//如果抢占了广告位了，同时更改
}	
...
<form name="myform" method="post" action="?action=modify">
  <?php
$rs=query("select * from caicaicms_textadv where id='$id'");
$row=fetch_array($rs);
?>
