...
if(!empty($_POST['id'])){
    ...
    $id=$_POST['id'][$i];
	$sql="select passed from caicaicms_special where id ='$id'";
	$rs = query($sql); 
	$row = fetch_array($rs);
		if ($row['passed']=='0'){
		query("update caicaicms_special set passed=1 where id ='$id'");
		}else{
		query("update caicaicms_special set passed=0 where id ='$id'");
		}
	}
}else{
...
<?php
$str="";	
$sql="select * from caicaicms_specialclass where parentid=0 order by xuhao";
$rs = query($sql); 
...
$sql="select id from caicaicms_special where id<>0 ";
$sql2='';
if ($shenhe=="no") {  		
...
$sql="select * from caicaicms_special where id<>0 ";
$sql=$sql.$sql2;
...
?>
      ...
      <div class="border"> <input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
        ...
        <input name="pagename" type="hidden"  value="special_manage.php?b=<?php echo $b?>&shenhe=<?php echo $shenhe?>&page=<?php echo $page ?>">      <input name="tablename" type="hidden"  value="caicaicms_special"></div>
</form>
