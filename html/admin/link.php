...
query("INSERT INTO caicaicms_link (bigclassid,sitename,url,logo,content,passed,elite,sendtime)VALUES('$bigclassid','$FriendSiteName','$url','$logo','$content','$passed','$elite','".date('Y-m-d H:i:s')."')");
}elseif ($_REQUEST["action"]=="modify") {
checkadminisdo("friendlink_modify");
query("update caicaicms_link set bigclassid='$bigclassid',sitename='$FriendSiteName',url='$url',logo='$logo',content='$content',passed='$passed',elite='$elite',sendtime='".date('Y-m-d H:i:s')."' where id='$id'");
}
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      ...
      <td class="border"> 
        ...
        $row=num_rows($rs);
		if (!$row){
			echo "<a href='class.php?tablename=caicaicms_linkclass'>添加类别</a>";
		}else{
		?>
...
<?php
$sql="select * from caicaicms_link where id='$id'";
$rs=query($sql);
$row=fetch_array($rs);
?>
<form action="?do=save" method="post" name="myform" id="myform" onSubmit="return CheckForm();">
  <table width="100%" border="0" cellspacing="0" cellpadding="5">
    <tr> 
      ...
      <td class="border"> 
        <?php
		$sqln = "select classid,classname from caicaicms_linkclass order by xuhao asc";
	    $rsn=query($sqln);
        $rown=num_rows($rsn);
		if (!$rown){
			echo "<a href='class.php?tablename=caicaicms_linkclass'>添加类别</a>";
