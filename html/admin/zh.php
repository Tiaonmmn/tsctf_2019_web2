...
query("INSERT INTO caicaicms_zh (bigclassid,title,address,timestart,timeend,content,passed,elite,sendtime)VALUES('$bigclassid','$title','$address','$timestart','$timeend','$content','$passed','$elite','".date('Y-m-d H:i:s')."')");
}elseif ($_REQUEST["action"]=="modify") {
checkadminisdo("zh_modify");
query("update caicaicms_zh set bigclassid='$bigclassid',title='$title',address='$address',timestart='$timestart',timeend='$timeend',content='$content',passed='$passed',elite='$elite',sendtime='".date('Y-m-d H:i:s')."' where id='$id'");
}
  ...
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      ...
      <td class="border">   
        ...
        $row=num_rows($rs);
		if (!$row){
			echo "<a href='class.php?tablename=caicaicms_zhclass'>请先添加类别</a>";
		}else{
		?>
...
<?php
$sql="select * from caicaicms_zh where id='$id'";
$rs=query($sql);
$row=fetch_array($rs);
?>
<form action="?do=save" method="post" name="myform"  id="myform" onSubmit="return CheckForm();"> 
  <table width="100%" border="0" cellpadding="5" cellspacing="0">
    <tr> 
      ...
      <td class="border"> 
	   <?php
		$sqln = "select classid,classname from caicaicms_zhclass order by xuhao asc";
	    $rsn=query($sqln);
        $rown=num_rows($rsn);
		if (!$rown){
			echo "<a href='class.php?tablename=caicaicms_zhclass'>请添加类别</a>";
		}else{
		?>
		<select name="bigclassid" id="bigclassid">
                <option value="" selected="selected">请选择类别</option>
