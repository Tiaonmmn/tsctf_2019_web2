...
if ($action=="del"){
	 if (strpos($id,",")>0){
		$sql="delete from caicaicms_bad where id in (". $id .")";
	}else{
		$sql="delete from caicaicms_bad where id='$id'";
	}
...
if ($action=="lockip"){
	 if (strpos($id,",")>0){
		$sql="update  caicaicms_bad set lockip=1 where id in (". $id .")";
	}else{
		$sql="update  caicaicms_bad set lockip=1 where id='$id'";
	}
query($sql);
echo "<script>location.href='showbad.php'</script>";
}
...
<?php
$sql="select * from caicaicms_bad order by id desc";
$rs=query($sql);
$row=num_rows($rs);	 
if (!$row){
echo "暂无信息";
...
?>
    ...
    <div class="border"> <label><input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
        ...
        <input name="tablename" type="hidden"  value="caicaicms_bad">
  </div>
