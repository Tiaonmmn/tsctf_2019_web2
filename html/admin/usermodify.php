...
$FoundErr=0;
	$sql="select * from caicaicms_user where id='$id'";
	$rs=query($sql);
	$row=num_rows($rs);
	if (!$row){
	$FoundErr=1;
	$errmsg=$errmsg . "<li>找不到指定的用户！</li>";
		...
		checkadminisdo("user_modify");
			...
			if ($FoundErr==0){
			query("update caicaicms_user set usersf='$usersf',sex='$sex',email='$email',homepage='$homepage',comane='$comane',content='$gsjj',
			...
			groupid='$groupid',totleRMB='$totleRMB',startdate='$startdate',enddate='$enddate',elite='$elite' where id='$id' ");	
				...
				if ($password!="") {
				query("update caicaicms_user set password='".md5($password)."',passwordtrue='$password' where id='$id' ");
				}
				if ($qqid==0) {
				query("update caicaicms_user set qqid='' where id='$id' ");
				}
				if ($groupid!=$oldgroupid){
					query("update caicaicms_main set groupid=" . $groupid . " where editor='" . $row["username"] . "'");
				}
				if ($qq<>$oldqq) {
				query("update caicaicms_main set qq='" . $qq . "' where editor='" . $row["username"] . "'");
				}
				if ($comane<>$oldcomane){
				query("update caicaicms_main set comane='" . $comane ."' where editor='" . $row["username"] . "'");
				}
				if ($renzheng<>$oldrenzheng) {
				query("update caicaicms_main set renzheng=" . $renzheng . " where editor='" . $row["username"] . "'");
				}
...
<FORM name="myform" action="?" method="post" onSubmit="return checkform()">
  <table width="100%" border="0" align="center" cellpadding="5" cellspacing="0">
    ...
    <tr> 
      ...
      <td class="border"> 
	  <?php
$sqln = "select classid,classname from caicaicms_userclass where parentid<>'0' order by xuhao asc";
$rsn=query($sqln);
?>
...
function changelocation(locationid){
	...
	?>
     <option value="<?php echo $rown["classid"]?>" <?php if ($rown["classid"]==$row["bigclassid"]) { echo "selected";}?>><?php echo $rown["classname"]?></option>
      ...
      <?php
$sqln="select classid,classname from caicaicms_userclass  where parentid='" .$row["bigclassid"]."' order by xuhao asc";
$rsn=query($sqln);
