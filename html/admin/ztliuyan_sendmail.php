...
if (strpos($id,",")>0){
$sql="select * from caicaicms_guestbook where saver is not null and  id in (". $id .")";
}else{
$sql="select * from caicaicms_guestbook where saver is not null and id='".$id."'";
}
$rs=query($sql);
$row=num_rows($rs);
if (!$row){		
...
while($row=fetch_array($rs)){
		$rsn=query("select username,sex,email,somane from caicaicms_user where username='".$row["saver"]."'");
		$rown=num_rows($rsn);
		if ($rown){
