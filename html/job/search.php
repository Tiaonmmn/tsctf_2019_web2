...
$bigclassname='';
if ($b<>0){
$sql="select classname from caicaicms_jobclass where classid='".$b."'";
$rs=query($sql);
$row=fetch_array($rs);
...
$smallclassname='';
if ($s<>0) {
$sql="select classname from caicaicms_jobclass where classid='".$s."'";
$rs=query($sql);
$row=fetch_array($rs);
...
checkid($page);
		...
		$str="";
        $sql = "select * from caicaicms_jobclass where parentid='0'";
        $rs=query($sql);
		$row=num_rows($rs);
		if (!$row){
		$str= "请先添加类别名称。";
		...
		$str="";
        $sql="select * from caicaicms_jobclass where parentid='" .$b. "' order by xuhao asc";
        $rs=query($sql);
		$row=num_rows($rs);
		if ($row){
			while($row=fetch_array($rs)){
...
$sql="select count(*) as total from caicaicms_job where passed<>0 ";
$sql2='';
switch ($yiju){
	case "Pname";
