...
//判断查看代理条数
$rslookedlsnumber=query("select looked_dls_number_oneday from caicaicms_usergroup where groupid=(select groupid from caicaicms_user where username='".$username."')");
$rown=fetch_array($rslookedlsnumber);
$lookedlsnumber=$rown["looked_dls_number_oneday"];
$rslookedlsnumbers=query("select looked_dls_number_oneday from caicaicms_looked_dls_number_oneday where username='".$username."' and  timestampdiff(day,sendtime,now()) < 3600*24 ");
...
$rslooked=query("select * from caicaicms_looked_dls_number_oneday where username='".$username."'");
	$rown=num_rows($rslooked);
	if (!$rown){
	query("insert into caicaicms_looked_dls_number_oneday (looked_dls_number_oneday,username,sendtime)values(1,'".$username."','".date('Y-m-d H:i:s')."') ");
	}else{
		if (time()-strtotime($rown["sendtime"])<3600*24){
		query("update caicaicms_looked_dls_number_oneday set looked_dls_number_oneday=looked_dls_number_oneday+".$i." where username='".$username."'");
		}else{
		query("update caicaicms_looked_dls_number_oneday set looked_dls_number_oneday=".$i.",sendtime='".date('Y-m-d H:i:s')."' where username='".$username."'");
		}
	}
...
if (strpos($id,",")>0){
$sql="select * from caicaicms_dl where passed=1 and id in (". $id .") order by id desc";
}else{
$sql="select * from caicaicms_dl where passed=1  and id='$id'";
}	
