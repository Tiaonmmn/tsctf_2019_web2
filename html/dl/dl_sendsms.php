...
$sql="select tel from caicaicms_dl where passed=1 and id in (".@$_COOKIE['dlid'].") order by id asc limit $n,$size";
}else{
$sql="select tel from caicaicms_dl where passed=1  and id='".@$_COOKIE['dlid']."'";
}
	$rs=query($sql); 
	$row=num_rows($rs); 
	if ($row){
