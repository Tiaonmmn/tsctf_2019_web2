...
$sql_n="select content from caicaicms_msg where elite=1";
$rs_n=query($sql_n);
$row_n=num_rows($rs_n);
if (!$row_n){
...
$sql="select email from caicaicms_dl where passed=1 and id in (". @$_COOKIE['dlid'] .") order by id asc limit $n,$size";
}else{
$sql="select email from caicaicms_dl where passed=1  and id='".@$_COOKIE['dlid']."'";
}
