...
if ($_COOKIE["UserName"]<>''){
$rs=query("select looked_dls_number_oneday from caicaicms_usergroup where groupid=(select groupid from caicaicms_user where username='".$_COOKIE["UserName"]."')");
$row=fetch_array($rs);
if ($row){
...
$rs=query("select looked_dls_number_oneday from caicaicms_looked_dls_number_oneday where username='".$_COOKIE["UserName"]."' and sendtime='".date('Y-m-d')."'");
$row=fetch_array($rs);
if ($row){
...
$sql="select * from caicaicms_dl where id='$dlid'";
$rs=query($sql);
$row=fetch_array($rs);
if (!$row){
echo showmsg("不存在相关信息！");
}else{
query("update caicaicms_dl set hit=hit+1 where id='$dlid'");
$saver=$row["saver"];
$cpid=$row["cpid"];
...
$rs=query("select classname,classzm from caicaicms_zsclass where classid='".$bigclassid."'");
$row=fetch_array($rs);
if ($row){
...
$looked=0;
$str="";		       
$sql="select groupid,totleRMB from caicaicms_user where username='".$_COOKIE["UserName"]."'";
$rs=query($sql);
$row=fetch_array($rs);				
$totleRMB=$row["totleRMB"];
$sqllooked="select * from caicaicms_looked_dls where dlsid=".$dlid." and username='".$_COOKIE["UserName"]."'";
...
$rowlooked=num_rows($rslooked);
	...
	$str=$str."</div>";	       
		...
		query("update caicaicms_user set totleRMB=totleRMB-".jf_look_dl." where username='".$_COOKIE["UserName"]."'");//查看时扣除积分
		query("insert into caicaicms_looked_dls(dlsid,username) values('$dlid','".$_COOKIE["UserName"]."')") ; //付分查看的写入记录表中
		query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('".$_COOKIE['UserName']."','查看".channeldl."信息','-".jf_look_dl."','<a href=/dl/show.php?id=$dlid>$dlid</a>','".date('Y-m-d H:i:s')."')");//写入冲值记录
		$str=$str.$showlx;
		}else{
...
case "No";
	...
	$str=$str."</div>";
			...
			$sql="select * from caicaicms_looked_dls_number_oneday where username='".$_COOKIE["UserName"]."'";
			$rs=query($sql);
			$row=fetch_array($rs);
			if (!$row){
			query("Insert into caicaicms_looked_dls_number_oneday(looked_dls_number_oneday,username,sendtime) values(1,'".$_COOKIE["UserName"]."','".date('Y-m-d')."')") ; //如果没有记录，加入新记录
			}else{//如果有记录更新记录
				if (date("Y-m-d",strtotime($row['sendtime']))==date('Y-m-d')){//如果当天查看过，查看数加一。
				query("update caicaicms_looked_dls_number_oneday set looked_dls_number_oneday=looked_dls_number_oneday+1 where username='".$_COOKIE["UserName"]."'");
				}else{//如果不是当天查看的查看数设为一，日期设为当天
				query("update caicaicms_looked_dls_number_oneday set looked_dls_number_oneday=1,sendtime='".date('Y-m-d')."' where username='".$_COOKIE["UserName"]."'");
				}
			}	
...
$str="";
if ($saver<>""){
	$rs=query("select img,comane,id from caicaicms_user where username='".$saver."'");
	$row=fetch_array($rs);
	if ($row){
