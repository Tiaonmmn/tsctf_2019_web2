...
$n=1;
switch ($channel){
	case "zh":$sql="select classname,classid from caicaicms_zhclass  order by xuhao";
	break;
	case "zx":$sql="select classid,classname from caicaicms_zxclass where isshow=1 and parentid=0 order by xuhao asc";
	break;
	case "special":$sql="select classid,classname from caicaicms_specialclass where isshow=1 and parentid=0 order by xuhao asc";
	break;
	case "job":$sql="select classname,classid from caicaicms_jobclass where parentid=0 order by xuhao";
	break;
	case "company":$sql="select classid,classname from caicaicms_userclass order by xuhao asc";
	break;
	default:$sql="select classname,classid,classzm from caicaicms_zsclass where parentid=0 order by xuhao";
	}
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
