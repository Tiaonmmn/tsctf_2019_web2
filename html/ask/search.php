...
$smallclassname="";
if ($b<>0){
$sql="select * from caicaicms_askclass where classid='$b'";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
...
if ($s<>0) {
$sql="select * from caicaicms_askclass where classid='$s'";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
...
function formbigclass(){
		$str="";
        $sql = "select * from caicaicms_askclass where parentid=0";
        $rs=query($sql);
		$row=num_rows($rs);
		if (!$row){
		$str= "请先添加类别名称。";
...
$sql="select count(*) as total from caicaicms_ask where passed<>0 ";
$sql2='';
if ($b<>0){
...
$rs_answer_num = query("select count(*) as total from caicaicms_answer where about='".$row["id"]."' ");
