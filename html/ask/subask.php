...
$n=1;
$str='';
$sql="select classid,classname from caicaicms_askclass where isshow=1 and parentid=0 order by xuhao asc";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
while($row=fetch_array($rs)){
	...
	$nn=1;
	$sqln="select classid,classname from caicaicms_askclass where parentid=".$row["classid"]." order by xuhao";
	$rsn=query($sqln);
	$rown=num_rows($rsn);
	if ($rown){
...
if ($b<>""){
$n=1;
$sql="select classid,classname from caicaicms_askclass where parentid='".$b."' order by xuhao";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
$str="";
...
$sql="select content,id,title,img from caicaicms_ask where bigclassid='$b' ";
if ($s!=0){
