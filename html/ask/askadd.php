...
$sql="select * from caicaicms_askclass where parentid='".$b."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
				?>
...
$sql="select * from caicaicms_askclass where parentid=" .@$_SESSION["bigclassid"]." order by xuhao asc";
$rs=query($sql);
	while($row = fetch_array($rs)){
	?>
...
if ($title<>''){
$isok=query("Insert into caicaicms_ask(bigclassid,smallclassid,title,content,img,jifen,editor,passed,sendtime) values('$bigclassid','$smallclassid','$title','$content','$img','0','未登陆用户',0,'".date('Y-m-d H:i:s')."')");
}  
if ($isok){
