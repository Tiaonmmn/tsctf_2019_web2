...
if (isset($_COOKIE["caicaicmscpid"])){
	if ($cpid<>$_COOKIE["caicaicmscpid"]){
	setcookie("caicaicmscpid",$cpid.",".$_COOKIE["caicaicmscpid"],time()+3600*24*360);
	}else{
	setcookie("caicaicmscpid",$cpid,time()+3600*24*360);
	}
}else{
setcookie("caicaicmscpid",$cpid,time()+3600*24*360);
}

$sql="select * from caicaicms_main where id='$cpid'";
$rs=query($sql);
$row=fetch_array($rs);
if (!$row){
...
query("update caicaicms_main set hit=hit+1 where id='$cpid'");
$editor=$row["editor"];
$cpmc=$row["proname"];
$imgbig=$row["img"];
...
$smallclasszm='###';
if ($smallclassid<>0){
$rs=query("select classname,classzm from caicaicms_zsclass where classid='".$smallclassid."'");
$row=fetch_array($rs);
if ($row){
...
$sql="select * from caicaicms_user where username='".$editor."'";
$rs=query($sql);
$row=fetch_array($rs);
...
$rsdl=query("select id from caicaicms_dl where cpid=$cpid and passed=1");
$rowdl=num_rows($rsdl);
	if ($rowdl){
	return "<b>".$rowdl."</b>";
	}else{
...
$rs=query("select * from caicaicms_dl where cpid=$cpid and passed=1 order by id desc");
$row=num_rows($rs);
if ($row){
$list2='';
...
$rs=query("select img,title,passed,editor from caicaicms_licence where editor='" .$editor. "' and passed=1");
$row=num_rows($rs);
if ($row){
$n=0;
$licence2='';
...
$rsn=query("select comane,somane,phone,email from caicaicms_user where username='".trim($_COOKIE["UserName"])."'");
