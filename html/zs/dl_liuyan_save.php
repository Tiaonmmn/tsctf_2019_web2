...
$rs=query("select groupid from caicaicms_user where username='".$saver."' ");
$row=fetch_array($rs);
$savergroupid=$row['groupid'];
if ($cp<>'' && $dlsname<>'' && $tel<>''){
//$rs=query("select * from caicaicms_dl where dlsname='$dlsname' and tel='$tel' and saver='$saver' and cpid='$cpid'");
//$row=num_rows($rs);
//if ($row){
//echo "<script>alert('您已留过言了！');history.back(-1)<//script>";
//}else{
$isok=query("insert into caicaicms_dl (cp,cpid,classid,province,city,content,company,companyname,dlsname,tel,email,editor,saver,savergroupid,sendtime)values('$cp','$cpid','$bigclassid','$province','$city','$contents','$company','$companyname','$dlsname','$tel','$email','".@$_COOKIE["UserName"]."','$saver','$savergroupid','".date('Y-m-d H:i:s')."')");
setcookie("dlliuyan",$saver,time()+3600,"/");//供留言后显示联系方式处用
$dlid=insert_id();

$rsn=query("select id,username,sex,email,mobile,somane from caicaicms_user where username='".$saver."'");
