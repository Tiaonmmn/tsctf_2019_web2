...
if (!isset($_COOKIE["caicaicmscpid"])){
$str="暂无记录";
}else{
$cpid=$_COOKIE["caicaicmscpid"];
	if (strpos($cpid,",")>0){
		$cpid=str_replace(" ","",$cpid);
		$cpid=str_replace("deleted","",$cpid);//cookie会出现deleted的情况
		$sql="select id,proname,img from caicaicms_main where id in (".$cpid.")";
	}else{
	checkid($cpid);
	$sql="select id,proname,img from caicaicms_main where id='$cpid' ";
	}
$n=1;
...
$str="";
$n=1;
$sql="select classname,classid,classzm from caicaicms_zsclass where parentid=0 order by xuhao";
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
$str="暂无分类";
...
$bigclassid='';
if ($b<>""){
$rs=query("select classid from caicaicms_zsclass where classzm='".$b."'");
$row=fetch_array($rs);
if ($row){
...
$n=1;
if ($num<>""){
$sql="select classname,classid,classzm from caicaicms_zsclass where parentid='". $bigclassid ."' order by xuhao limit 0,$num";
}else{
$sql="select classname,classid,classzm from caicaicms_zsclass where parentid='". $bigclassid ."' order by xuhao";
}
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
...
$n=1;
$str="";
	$sql="select id,proname,sendtime,passed,elite,hit,city,comane,userid from caicaicms_main where passed=1 ";
	if ($bigclassid<>0) {
...
$n=1;
$str="";
$sql="select cpid from caicaicms_dl where passed=1 and cpid<>0 ";
if ($classid<>"") {
...
//}
$str=$str. "<div>";
	$sqln="select proname,id,img,prouse from caicaicms_main where id=".$row["cpid"]." ";
	$rsn=query($sqln);
	$r=num_rows($rsn);
	if ($r){
	$r=fetch_array($rsn);
...
//checkid($cpid);//非用户输入值，可以用不判断
	$sql="select id,proname,img,sendtime,passed,elite,hit,city,comane,userid from caicaicms_main where passed=1 ";
