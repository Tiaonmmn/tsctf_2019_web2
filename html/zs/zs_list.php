...
setcookie("caicaicmscpid","",time()-3600);
...
$bigclassid=0;
if ($b<>""){
$sql="select * from caicaicms_zsclass where classzm='".$b."'";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
...
$smallclassid=0;
if ($s<>"") {
$sql="select * from caicaicms_zsclass where classzm='".$s."'";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
...
$sql="select id,proname,prouse,shuxing_value,img,tz,link,province,city,xiancheng,province_user,city_user,xiancheng_user,sendtime,editor,elite,userid,comane,qq,groupid,renzheng from caicaicms_main where passed=1 ";
$sql=$sql.$sql2;
