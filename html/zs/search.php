...
checkid($page);
		...
		$str="";
        $sql = "select classzm,classname from caicaicms_zsclass where parentid=0";
        $rs=query($sql);
		...
		if ($b!=0){
		$str="";
        $sql="select classzm,classname from caicaicms_zsclass where parentid='" .$b. "' order by xuhao asc";
        $rs=query($sql);
			while($row=fetch_array($rs)){
...
$sql="select count(*) as total from caicaicms_main where passed<>0 ";
$sql2='';
if ($keyword!=''){
	switch ($yiju){
	case "Pname";
	...
	//strwhere=" editor in (select username from caicaicms_user where comane like '%"&keyword&"%') "
	break;
	}
}	
if ($bigclassid<>0){
...
if ($vip=="yes") {
$sql2=$sql2." and editor in (select username from caicaicms_user where groupid>1)";
}
$rs =query($sql.$sql2);
$row = fetch_array($rs);
$totlenum = $row['total'];
...
$sql="select id,proname,prouse,shuxing_value,img,tz,link,province,city,xiancheng,province_user,city_user,xiancheng_user,sendtime,editor,elite,userid,comane,qq,groupid,renzheng,tag from caicaicms_main where passed=1 ";
$sql=$sql.$sql2;
...
$list2='';
	...
	if (showdlinzs=="Yes") {
	$rsn=query("select id from caicaicms_dl where cpid=".$row["id"]." and passed=1");
