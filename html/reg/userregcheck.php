...
$msg= "请输入用户名";
	...
	$sqlreg="select username from caicaicms_usernoreg where username='".$id."'";
	$rs =query($sqlreg);
	...
	$sqlreg="select username from caicaicms_user where username='".$id."'";
	$rs =query($sqlreg);
	$row= num_rows($rs);//返回记录数
		if($row){ 
		$founderr=1;
...
if ($id==''){
	...
	$sql="select comane from caicaicms_user where comane='".$id."' ";
	$rs = query($sql); 
	$row= num_rows($rs);//返回记录数
	if($row){
	 	$founderr=1;
	...
	$sql="select comane from caicaicms_usernoreg where comane='".$id."'";
	$rs = query($sql); 
	$row= num_rows($rs);//返回记录数
	if($row){
	 		$founderr=1;
...
if ($id==''){
	...
	if (allowrepeatreg=="No"){
	$sql="select phone from caicaicms_user where phone='".$id."'";
	$rs = query($sql); 
	$row= num_rows($rs);//返回记录数
		if($row){
	 	$founderr=1;
...
if ($id==''){
	...
	if (allowrepeatreg=="No"){
	$sql="select email from caicaicms_user where email='".$id."'";
	$rs = query($sql); 
	$row= num_rows($rs);//返回记录数
		if($row){
	 	$founderr=1;
