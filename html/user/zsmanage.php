...
<?php
$sql="select refresh_number,groupid from caicaicms_usergroup where groupid=(select groupid from caicaicms_user where username='".$username."')";
$rs = query($sql); 
if(empty($rs)){
$refresh_number=3;
}else{
...
$sql="select refresh,sendtime from caicaicms_main where editor='".$username."' ";
$rs = query($sql);
$row = fetch_array($rs);
if ($action=="refresh") {
    if ($row["refresh"]< $refresh_number){
	query("update caicaicms_main set sendtime='".date('Y-m-d H:i:s')."',refresh=refresh+1 where editor='".$username."'");
	...
	query("update caicaicms_main set refresh=0 where editor='".$username."'");
  	}
}
...
$sql="select count(*) as total from caicaicms_main where editor='".$username."' ";
$sql2='';
if (isset($cpmc)){
...
$sql="select id,bigclassid,smallclassid,smallclassids,proname,refresh,img,province,city,xiancheng,sendtime,elite,passed,elitestarttime,eliteendtime,tag from caicaicms_main where editor='".$username."' ";
$sql=$sql.$sql2;		
...
?>
    <tr class="trcontent"> 
      ...
      <td align="center">
	  <?php
	$sqln="select classname from caicaicms_zsclass where classid='".$row["bigclassid"]."' ";
	$rsn =query($sqln); 
	$rown = fetch_array($rsn);
	echo $rown["classname"]."<br/>";
	
	if (strpos($row["smallclassids"],",")>0){
	$sqln="select classname from caicaicms_zsclass where parentid='".$row["bigclassid"]."' and classid in (".$row["smallclassids"].") ";
	$rsn =query($sqln);
	while($rown = fetch_array($rsn)){
	echo " [".$rown["classname"]."]";
	}
	}else{
	$sqln="select classname from caicaicms_zsclass where classid='".$row["smallclassid"]."' ";
	$rsn =query($sqln); 
	$rown = fetch_array($rsn);
	echo $rown["classname"];
	}
...
<?php echo showpage()?> 
          ...
          <input name="tablename" type="hidden" id="tablename" value="caicaicms_main" />
        </div>
</form>
<?php
}
