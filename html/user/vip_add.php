...
if ($action=="modify"){
    ...
    $rs=query("Select RMB from caicaicms_usergroup where groupid='$groupid'");
	$row=fetch_array($rs);
	$totleRMB=$sj*$row["RMB"];
	
	$rs=query("select * from caicaicms_user where username='" . $username ."'");
	$row=num_rows($rs);
	if (!$row){
		$FoundErr=1;
	...
	$row=fetch_array($rs);
		if ($row["groupid"]>=$groupid){
			...
			query("update caicaicms_user set groupid='$groupid',startdate='$startdate',enddate='$enddate',totleRMB=totleRMB-".$totleRMB." where username='" . $username ."'");
			query("Update caicaicms_main set groupid='" . $groupid . "' where editor='" . $username . "'");
			query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime)values('$username','会员升级费用','$totleRMB','服务时间：".$startdate."-".$enddate."','".date('Y-m-d H:i:s')."')");
...
<table width="100%" border="0" cellpadding="3" cellspacing="1">
            ...
            <tr> 
              <td align="right" class="border2">你当前所属用户组：</td>
                    <td class="border2"> 
                      <?php
$rs=query("Select groupname from caicaicms_usergroup where groupid=(select groupid from caicaicms_user where username='".$username."')");
$row=fetch_array($rs);
echo $row["groupname"];
?>
