...
$sql="select id,proname,eliteendtime,tag from caicaicms_main where tag='".$tag."' and id<>'$id'";
$rs = query($sql); 
$row=num_rows($rs);
if ($row){
$row = fetch_array($rs);
...
case 'yes':
if (jifen=="Yes"){
$sqln="select totleRMB from caicaicms_user where username='".$username."'";
$rsn =query($sqln);
$rown = fetch_array($rsn);
	if ($rown["totleRMB"]>=$jfpay){
	query("update caicaicms_user set totleRMB=totleRMB-$jfpay where username='".$username."'");
	query("update caicaicms_main set elitestarttime='".date('Y-m-d')."',eliteendtime='$eliteendtime',tag='$tag',elite=1 where id='$id'");
	query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('$username','".channelzs."信息投标','-$jfpay','产品ID：<a href=zsmanage.php?id=$id>$id</a>','".date('Y-m-d H:i:s')."')");
...
$sql="select id,editor,proname,eliteendtime,tag from caicaicms_main where id='$id'";
$rs = query($sql); 
$row = fetch_array($rs);
