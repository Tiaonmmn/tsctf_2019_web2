...
exit;
 	...
 	if (strpos($id,",")>0){
	//$sql="select * from caicaicms_dl where passed=1 and saver='".$username."' and id in (". $id .") and id in(select max(id) from caicaicms_dl group by tel)";
	$sql="select * from caicaicms_dl where passed=1 and saver='".$username."' and id in (". $id .")";
	}else{
	$sql="select * from caicaicms_dl where passed=1 and saver='".$username."' and id='$id' order by id desc";
	}
