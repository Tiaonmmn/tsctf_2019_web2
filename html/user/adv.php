...
query("update caicaicms_textadv set adv='$adv',company='$company',advlink='$advlink',img='$img',passed=0 where username='".$_COOKIE["UserName"]."'");
...
if ($oldimg<>$img){
	...
	//$rs=query("select * from caicaicms_ad where username='".$_COOKIE["UserName"]."'");
    //$row=num_rows($rs);
	//if ($row){
	//query("update caicaicms_ad set title='<b>新的广告内容正在审核中...</b>',link='###' where username='".$_COOKIE["UserName"]."'");
	//}
...
if ($action=="add"){
checkstr($img,"upload");//入库前查上传文件地址是否合格
query("insert into caicaicms_textadv (adv,company,advlink,img,username,passed,gxsj)values('$adv','$company','$advlink','$img','".$_COOKIE["UserName"]."',0,'".date('Y-m-d H:i:s')."') ");
        ...
        <table width="100%" border="0" cellpadding="3" cellspacing="1">
          <tr> 
            ...
            <td width="80%" class="border"> <input name="adv" type="text" id="adv" class="biaodan" value="<?php echo $row["adv"]?>" size="40" maxlength="15"> 
              <?php
		$rsn=query("select id,comane from caicaicms_user where username='".$_COOKIE["UserName"]."'");
