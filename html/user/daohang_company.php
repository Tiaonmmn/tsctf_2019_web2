...
checkstr($img,"upload");//入库前查上传文件地址是否合格
			query("update caicaicms_user set bigclassid='$b',smallclassid='$s',content='$content',img='$img',
			...
			if ($oldcontent=="" || $oldcontent=="&nbsp;"){//只有第一次完善时加分，修改信息不计分，这里需要加验证，不许改为空，防止刷分
				query("update caicaicms_user set totleRMB=totleRMB+".jf_addreginfo." where username='".$username."'");
				query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('$username','完善注册信息','+".jf_addreginfo."','+".jf_addreginfo."','".date('Y-m-d H:i:s')."')");
...
new PCAS('province', 'city', 'xiancheng', '<?php echo $row['province']?>', '<?php echo $row["city"]?>', '<?php echo $row["xiancheng"]?>');
          ...
          <tr> 
            ...
            <td class="border">
			<?php
$sqln = "select classid,parentid,classname from caicaicms_userclass where parentid<>'0' order by xuhao asc";
$rsn=query($sqln);
?>
...
<option value="<?php echo $rown["classid"]?>" <?php if ($rown["classid"]==$row["bigclassid"]) { echo "selected";}?>><?php echo $rown["classname"]?></option>
      ...
      <?php	  
$sqln="select * from caicaicms_userclass where parentid='" .$row["bigclassid"]."' order by xuhao asc";
$rsn=query($sqln);
while($rown = fetch_array($rsn)){
?>
