...
$sql="select * from caicaicms_usermessage where editor='".$username."' ";
$rs = query($sql); 
...
<input name="chkAll" type="checkbox" id="chkAll" onclick="CheckAll(this.form)" value="checkbox" />
          ...
          <input name="tablename" type="hidden" id="tablename" value="caicaicms_usermessage" />
        </div>
</form>
<?php
}
...
$sql="select content,editor from caicaicms_usermessage where content='".$content."'";
$rs = query($sql); 
$row= num_rows($rs); 
if ($row){
echo "<script lanage='javascript'>alert('此信息已存在，请不要发布重复的信息！');</script>";
}else{
	$sql="select sendtime from caicaicms_usermessage where editor='".$editor."' order by id desc limit 1";
	$rs = query($sql); 
	$row= fetch_array($rs); 
	...
	query("Insert into caicaicms_usermessage(content,editor,sendtime) values('$content','$editor','".date('Y-m-d H:i:s')."')");
