...
if ($_POST["action"]=="add"){
$isok=query("Insert into caicaicms_dl(classid,cpid,cp,province,city,xiancheng,content,company,companyname,dlsname,tel,address,email,sendtime,editor) values('$classid',0,'$cp','$province','$city','$xiancheng','$content','$dlsf','$companyname','$truename','$tel','$address',
'$email','".date('Y-m-d H:i:s')."','$username')") ;  
$id=insert_id();	
	
}elseif ($_POST["action"]=="modify"){
$isok=query("update caicaicms_dl set classid='$classid',cp='$cp',province='$province',city='$city',xiancheng='$xiancheng',
...
function add(){
global $username;
$tablename="caicaicms_dl";//checkaddinfo中用
...
$sql="select * from caicaicms_dl where id='$id'";
$rs = query($sql); 
$row = fetch_array($rs);
        ...
        <table width="100%" border="0" cellpadding="3" cellspacing="1">
          ...
          <tr> 
            ...
            <td class="border2" ><select name="classid" class="biaodan">
                ...
                <?php
		$sqln="select * from caicaicms_zsclass where parentid=0";
		$rsn=query($sqln);
