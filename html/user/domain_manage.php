...
if ($action=="quxiao"){
	if ($username<>"" ){
	query("update caicaicms_userdomain set del=1 where username='$username'");
	}
...
if ($action=="del"){
	//if ($username<>"" ){
	//query("delete from caicaicms_userdomain where username='$username'") ;
	//}
	echo "<script>location.href='domain_manage.php'</script>";
}
?>
...
<?php
$sql="select * from caicaicms_userdomain where username='$username' and del=0";
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
