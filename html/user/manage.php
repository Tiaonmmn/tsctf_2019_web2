...
if ($action=="modify") {
			...
			$rsn=query("select * from caicaicms_user where phone='" . $phone . "' and username!='$username'");
			$r=num_rows($rsn);
			if ($r){
			$founderr=1;
			...
			query("update caicaicms_user set bigclassid='$b',smallclassid='$s',content='$content',img='$img',flv='$flv',province='$province',city='$city',
			...
			if ($oldflv<>$flv){
				...
				if ($qq<>$oldqq) {
				query("Update caicaicms_main set qq='" . $qq . "' where editor='" . $username . "'");
				}
