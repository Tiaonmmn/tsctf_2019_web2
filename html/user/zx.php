...
$sql="select title,editor from caicaicms_zx where title='".$title."'";
$rs = query($sql); 
$row= num_rows($rs); 
if ($row){
...
$isok=query("Insert into caicaicms_zx(bigclassid,bigclassname,smallclassid,smallclassname,title,link,laiyuan,keywords,description,groupid,jifen,content,img,editor,sendtime) values('$bigclassid','$bigclassname','$smallclassid','$smallclassname','$title','$link','$laiyuan','$keywords','$description',
'$groupid','$jifen','$content','$img','$username','".date('Y-m-d H:i:s')."')");  
$id=insert_id();		
}elseif ($_POST["action"]=="modify"){
$isok=query("update caicaicms_zx set bigclassid='$bigclassid',bigclassname='$bigclassname',smallclassid='$smallclassid',smallclassname='$smallclassname',title='$title',link='$link',laiyuan='$laiyuan',
...
$sql = "select classid,parentid,classname from caicaicms_zxclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
...
$sql="select classid,classname from caicaicms_zxclass where parentid='".$b."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
...
$sql="select classid,classname from caicaicms_zxclass where parentid='" .@$_COOKIE["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
	while($row = fetch_array($rs)){
	?>
...
$sqlzx="select * from caicaicms_zx where id='$id'";
$rszx =query($sqlzx); 
...
<form action="?do=save" method="post" name="myform" id="myform" onSubmit="return CheckForm();">
        <table width="100%" border="0" cellpadding="3" cellspacing="1">
          <tr> 
            ...
            <td width="82%" class="border2"> 
              <?php
$sql = "select parentid,classid,classname from caicaicms_zxclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
<script language = "JavaScript" type="text/JavaScript">
var onecount;
...
<select name="bigclassid" class="biaodan" onChange="changelocation(document.myform.bigclassid.options[document.myform.bigclassid.selectedIndex].value)" size="1">
    ...
    <option value="<?php echo $row["classid"]?>" <?php if ($row["classid"]==$rowzx["bigclassid"]) { echo "selected";}?>><?php echo $row["classname"]?></option>
                ...
                <option value="0">不指定小类</option>
                <?php
$sql="select classid,classname from caicaicms_zxclass where parentid='" .$rowzx["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
