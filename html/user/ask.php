...
$sql="select title,editor from caicaicms_ask where title='".$title."'";
$rs = query($sql); 
$row= num_rows($rs); 
if ($row){
...
$isok=query("Insert into caicaicms_ask(bigclassid,smallclassid,title,content,img,jifen,editor,passed,sendtime) values('$bigclassid','$smallclassid','$title','$content','$img','$jifen','$username',1,'".date('Y-m-d H:i:s')."')");
query("update caicaicms_user set totleRMB=totleRMB-".$jifen." where username='$username'");//减去发布者积分
query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('$username','提问花积分','-".jifen."','','".date('Y-m-d H:i:s')."')");//记录积分
$id=insert_id();		

}elseif ($_POST["action"]=="modify"){

$id=$_POST["id"];
$isok=query("update caicaicms_ask set bigclassid='$bigclassid',smallclassid='$smallclassid',title='$title',
...
query("update caicaicms_user set totleRMB=totleRMB-".$jifen_cha." where username='$username'");//减去发布者积分
query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('$username','提问提高悬赏花积分','-".$jifen_cha."','','".date('Y-m-d H:i:s')."')");//记录积分
}

}
passed("caicaicms_ask");
?>
 ...
 <select name="bigclassid" class="biaodan" onChange="changelocation(document.myform.bigclassid.options[document.myform.bigclassid.selectedIndex].value)" size="1">
	...
	?>		
          ...
          <tr id="trkeywords">
			...
			<?php	   
        $rs=query("select totleRMB from caicaicms_user where username='" .$_COOKIE["UserName"]. "'");
		$row=fetch_array($rs);
...
$sqlzx="select * from caicaicms_ask where id='$id'";
$rszx =query($sqlzx); 
