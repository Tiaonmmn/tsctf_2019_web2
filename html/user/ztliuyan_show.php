...
<body>
<?php
$sql="select * from caicaicms_guestbook where id='$id'";
$rs=query($sql);
$row=num_rows($rs);
if (!$row){
echo "不存在相关信息";
...
<div class="admintitle">展厅留言</div> 
 ...
 <?php
         switch  (check_user_power("look_dls_liuyan")){
			case "yes" ;
			query("update caicaicms_guestbook set looked=1 where id='$id'");
			...
			case "no";
			    if (jifen=="Yes"){
                    ...
                    $sql="select totleRMB from caicaicms_user where username='".$_COOKIE["UserName"]."'";
					$rsuser=query($sql);
					$rowuser=fetch_array($rsuser);
			        	if ($rowuser["totleRMB"]>=jf_lookmessage) {
						query("update caicaicms_user set totleRMB=totleRMB-".jf_lookmessage." where username='".$_COOKIE["UserName"]."'");//查看时扣除积分
						query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('".@$_COOKIE['UserName']."','查看展厅留言','-".jf_lookmessage."','<a href=ztliuyan_show.php?id=$id>$id</a>','".date('Y-m-d H:i:s')."')");//写入冲值记录
			       		query("update caicaicms_guestbook set looked=1 where id='$id'");
						echo showlx($dlsname,$tel,$email);
						}else{
