...
if ($_POST["action"]=="add"){
if ($cp<>'' && $truename<>'' && $tel<>''){
$isok=query("Insert into caicaicms_baojia(classid,cp,province,city,price,danwei,companyname,truename,tel,address,email,sendtime,editor) values('$classid','$cp','$province','$city','$price','$danwei','$companyname','$truename','$tel','$address','$email','".date('Y-m-d H:i:s')."','$username')") ;
$id=insert_id();	
}	
}elseif ($_POST["action"]=="modify"){
checkstr($tel,'tel');
$isok=query("update caicaicms_baojia set classid='$classid',cp='$cp',province='$province',city='$city',price='$price',danwei='$danwei',companyname='$companyname',truename='$truename',tel='$tel',address='$address',email='$email',sendtime='".date('Y-m-d H:i:s')."' where id='$id'");
}
passed("caicaicms_baojia",$classid);
    ...
    <tr> 
      ...
      <td class="border2">
	   <select name="classid" class="biaodan">
          ...
          <?php
		$sql="select * from caicaicms_zsclass where parentid=0";
		$rs=query($sql);
...
new PCAS('province', 'city', 'xiancheng', '', '', '');
	...
	<?php
	$sql="select * from caicaicms_user where username='".$username."'";
	$rs=query($sql);
	$row= fetch_array($rs);
	?>
...
$sql="select * from caicaicms_baojia where id='$id'";
$rs = query($sql); 
$row = fetch_array($rs);
        ...
        <table width="100%" border="0" cellpadding="3" cellspacing="1">
          ...
          <tr> 
            ...
            <td class="border2" ><select name="classid" class="biaodan">
                ...
                <?php
		$sqln="select classid,classname from caicaicms_zsclass where parentid=0";
		$rsn=query($sqln);
