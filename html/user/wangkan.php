...
$isok=query("Insert into caicaicms_wangkan(bigclassid,title,content,img,editor,sendtime) values('$bigclassid','$title','$content','$img','$username','".date('Y-m-d H:i:s')."')") ;
$id=insert_id();	
}elseif ($_POST["action"]=="modify"){
$isok=query("update caicaicms_wangkan set bigclassid='$bigclassid',title='$title',content='$content',img='$img',
editor='$username',sendtime='".date('Y-m-d H:i:s')."',passed=0 where id='$id'");
}		
passed("caicaicms_wangkan");
?>
...
function add(){
$tablename="caicaicms_wangkan";//checkaddinfo中用
        ...
        <table width="100%" border="0" cellpadding="3" cellspacing="1">
          <tr> 
            ...
            <td width="85%" class="border2"> <select name="bigclassid" id="bigclassid" class="biaodan">
                <option value="" selected="selected">请选择类别</option>
        <?php
		$rsn=query("select * from caicaicms_wangkanclass");
