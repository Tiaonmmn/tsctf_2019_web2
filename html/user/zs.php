...
$rs=query("select groupid,qq,comane,id,renzheng,province,city,xiancheng from caicaicms_user where username='".$username."'");
$row=fetch_array($rs);
$groupid=$row["groupid"];
$qq=$row["qq"];
$comane=$row["comane"];
...
//判断是不是重复信息
if ($_POST["action"]=="add" ){
$sql="select id from caicaicms_main where proname='".$proname."' and editor='".$username."' ";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
...
$sql="select id from caicaicms_main where proname='".$proname."' and editor='".$username."' and id<>".$cpid." ";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
showmsg('您已发布过该信息，请不要发布重复的信息！');
...
$isok=query("Insert into caicaicms_main(proname,bigclassid,smallclassid,smallclassids,szm,tz,prouse,sm,img,flv,province,city,xiancheng,
...
$isok=query("update caicaicms_main set proname='$proname',bigclassid='$bigclassid',smallclassid='$smallclassid',smallclassids='$smallclassids',szm='$szm',tz='$tz',prouse='$prouse',sm='$sm',
