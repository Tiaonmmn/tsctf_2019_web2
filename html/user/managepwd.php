...
WriteErrMsg('密码不能为空');
	...
	$sql="select password,email,mobile from caicaicms_user where username='" . $username . "'";
	$rs=query($sql);
	$row=fetch_array($rs);
	...
	query("update caicaicms_user set password='$password',passwordtrue='".trim($_POST["password"])."' where username='".$username."'");
