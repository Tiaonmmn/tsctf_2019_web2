...
$password=nostr($_COOKIE["PassWord"]);
	$rs=query("select id,usersf,lastlogintime from caicaicms_user where lockuser=0 and username='".$username."' and password='".$password."'");
	$row=num_rows($rs);
		if (!$row){
		...
		query("update caicaicms_user set loginip = '".getip()."' where username='".$username."'");//更新最后登录IP
			...
			query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('".$username."','每天登录用送积分','+".jf_login."','','".date('Y-m-d H:i:s')."')");
			}
