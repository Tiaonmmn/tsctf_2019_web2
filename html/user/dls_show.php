...
<div class="admintitle"> <?php echo channeldl?>信息</div> 
 ...
 <?php
         switch  (check_user_power("look_dls_liuyan")){
			case "yes" ;
			query("update caicaicms_dl set looked=1 where id='$id'");
			...
			case "no";
			    if (jifen=="Yes"){
					...
					$rowuser=fetch_array($rsuser);
						...
						query("insert into caicaicms_pay (username,dowhat,RMB,mark,sendtime) values('".@$_COOKIE['UserName']."','查看留言','-".jf_lookmessage."','<a href=dls_show.php?id=$id>$id</a>','".date('Y-m-d H:i:s')."')");//写入冲值记录
			       		query("update caicaicms_dl set looked=1 where id='$id'");
						echo showlx($dlsname,$company,$companyname,$tel,$email);
						}else{
