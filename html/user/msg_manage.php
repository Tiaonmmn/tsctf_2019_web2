...
if ($action=="elite"){
	if ($id<>"" ){
	query("Update caicaicms_msg set elite=0 ");//只有一条为1的
	query("update caicaicms_msg set elite=1 where id='$id'");
}
...
if ($action=="del"){
	if ($id<>"" ){
	query("delete from caicaicms_msg where id='$id'") ;
	}
	echo "<script>location.href='msg_manage.php'</script>";
}
?>
