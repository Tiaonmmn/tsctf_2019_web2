...
if ($action=="savedata" ){
	...
	if ($saveas=="add"){
	query("insert into caicaicms_msg (content)VALUES('$content') ");
	$go=1;
	}elseif ($saveas=="modify"){
	query("update caicaicms_msg set content='$content' where id='". $id."'");
	$go=1;
	}
}
?>
...
if ($action=="modify") {
$sql="select * from caicaicms_msg where id='".$id."'";
$rs=query($sql);
$row=fetch_array($rs);
?>
