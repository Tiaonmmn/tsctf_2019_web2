...
//判断大小类是否一致，修改产品时有用
if ($smallclassid<>0){
$sql="select * from caicaicms_zsclass where parentid='".$bigclassid."' and  classid='".$smallclassid."'";
$rs=query($sql);
$row=fetch_array($rs);
if (!$row){
...
$sql="select * from caicaicms_pp where ppname='".$proname."' and editor='".$username."' ";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
...
$sql="select * from caicaicms_pp where ppname='".$proname."' and editor='".$username."' and id<>'".$cpid."' ";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
showmsg('您已发布过该信息，请不要发布重复的信息！');
...
if ($_POST["action"]=="add"){
$isok=query("Insert into caicaicms_pp(ppname,bigclassid,smallclassid,sm,img,sendtime,editor,userid,comane) values('$proname','$bigclassid','$smallclassid','$sm','$img','".date('Y-m-d H:i:s')."','$username','$userid','$comane')") ;
$cpid=insert_id();		
}elseif ($_POST["action"]=="modify"){
$oldimg=trim($_POST["oldimg"]);
checkstr($oldimg,"upload");
$isok=query("update caicaicms_pp set ppname='$proname',bigclassid='$bigclassid',smallclassid='$smallclassid',sm='$sm',img='$img',sendtime='".date('Y-m-d H:i:s')."',editor='$username',userid='$userid',comane='$comane',passed=0 where id='$cpid'");
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="1">
                <tr> 
                  <td> <fieldset class="fieldsetstyle">
                    ...
                    <?php
        $sql = "select * from caicaicms_zsclass where parentid=0 order by xuhao asc";
		$rs = query($sql); 
		$n=1;
...
$sqln="select * from caicaicms_zsclass where parentid='$row[classid]' order by xuhao asc";
$rsn =query($sqln); 
$nn=1;
...
$sql="select * from caicaicms_pp where id='$id'";
$rs = query($sql); 
$row = fetch_array($rs);
            ...
            <td class="border2" > <table width="100%" border="0" cellpadding="0" cellspacing="1">
                <tr> 
                  <td> <fieldset class="fieldsetstyle">
                    <legend>请选择所属大类</legend>
                    <?php
        $sqlB = "select classid,classname from caicaicms_zsclass where parentid=0 order by xuhao asc";
		$rsB =query($sqlB); 
		$n=1;
...
echo "<fieldset class='fieldsetstyle'><legend>请选择所属小类</legend>";
$sqlS="select classid,classname from caicaicms_zsclass where parentid='$rowB[classid]' order by xuhao asc";
$rsS =query($sqlS); 
$nn=0;
