...
if ($_POST["action"]=="add"){
$isok=query("insert into caicaicms_licence(title,img,editor,sendtime) values('$title','$img','$username','".date('Y-m-d H:i:s')."')") ;
...
checkstr($oldimg,"upload");
	...
	$isok=query("update caicaicms_licence set title='$title',img='$img',sendtime='".date('Y-m-d H:i:s')."',passed=0 where id='$id'");
...
$sql="select * from caicaicms_licence where id='$id'";
$rs = query($sql); 
$row = fetch_array($rs);
