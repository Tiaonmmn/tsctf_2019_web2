...
$isok=query("insert into caicaicms_special(bigclassid,bigclassname,smallclassid,smallclassname,title,laiyuan,keywords,description,groupid,jifen,content,img,editor,sendtime)
...
$sqlzx="select * from caicaicms_special where id='$id'";
$rszx =query($sqlzx); 
...
<form action="?do=save" method="post" name="myform" id="myform" onSubmit="return CheckForm();">
        <table width="100%" border="0" cellpadding="3" cellspacing="1">
          <tr> 
            ...
            <td width="82%" class="border2"> 
              <?php
$sql = "select parentid,classid,classname from caicaicms_specialclass where parentid<>0 order by xuhao asc";
$rs=query($sql);
?>
<script language = "JavaScript" type="text/JavaScript">
var onecount;
...
<option value="<?php echo $row["classid"]?>" <?php if ($row["classid"]==$rowzx["bigclassid"]) { echo "selected";}?>><?php echo $row["classname"]?></option>
	...
	?>
                ...
                <option value="0">不指定小类</option>
                <?php
$sql="select * from caicaicms_specialclass where parentid='" .$rowzx["bigclassid"]."' order by xuhao asc";
$rs=query($sql);
while($row = fetch_array($rs)){
?>
