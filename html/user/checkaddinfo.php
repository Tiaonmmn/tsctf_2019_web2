...
$ErrMsg="";
$FoundErr=0;
$rs=query("select passed,usersf,username from caicaicms_user where username='".$_COOKIE["UserName"]."' ");
$row=fetch_array($rs);
...
//if ($tablename=="caicaicms_main" || $tablename=="caicaicms_licence") {
	//if ($row["usersf"]=="个人"){
		//$FoundErr=1;
		//$ErrMsg=$ErrMsg . "<li>个人用户不能发布此信息</li>";
	//}	
//}
$rs=query("select addinfo_number,addinfototle_number from caicaicms_usergroup where groupid=(select groupid from caicaicms_user where username='".$_COOKIE["UserName"]."')");
$row=fetch_array($rs);
if ($row){
