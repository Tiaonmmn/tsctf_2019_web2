...
if ($bigclassid!=0){
$rs = query("select classname from caicaicms_jobclass where classid='$bigclassid'");
$row= fetch_array($rs);
$bigclassname=$row["classname"];
}

if ($smallclassid !=0){
$rs = query("select classname from caicaicms_jobclass where classid='$smallclassid'");
$row= fetch_array($rs);
$smallclassname=$row["classname"];
}

$rs=query("select comane,id from caicaicms_user where username='".$username."'");
$row=fetch_array($rs);
$comane=$row["comane"];
$userid=$row["id"];
...
//判断大小类是否一致，修改产品时有用
if ($smallclassid<>0){ 
$sql="select * from caicaicms_jobclass where parentid='".$bigclassid."' and  classid='".$smallclassid."'";
$rs=query($sql);
$row=fetch_array($rs);
if (!$row){
...
//判断是不是重复信息
if ($_POST["action"]=="add" ){
$sql="select * from caicaicms_job where jobname='".$jobname."' and editor='".$username."' ";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
...
$sql="select * from caicaicms_job where jobname='".$jobname."' and editor='".$username."' and id<>'".$jobid."' ";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
showmsg('您已发布过这条信息，请不要发布重复的信息！','jobmanage.php');
...
if ($_POST["action"]=="add"){
$isok=query("Insert into caicaicms_job(jobname,bigclassid,smallclassid,sm,province,city,xiancheng,sendtime,editor,userid,comane) values('$jobname','$bigclassid','$smallclassid','$sm','$province','$city','$xiancheng','".date('Y-m-d H:i:s')."','$username','$userid','$comane')") ;
$jobid=insert_id();		
}elseif ($_POST["action"]=="modify"){
$isok=query("update caicaicms_job set jobname='$jobname',bigclassid='$bigclassid',smallclassid='$smallclassid',sm='$sm',
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr> 
                  <td> <fieldset class="fieldsetstyle">
                    <legend>请选择大类</legend>
                    <?php
        $sql = "select * from caicaicms_jobclass where parentid='0' order by xuhao asc";
		$rs = query($sql); 
		$n=1;
...
$sqln="select * from caicaicms_jobclass where parentid='$row[classid]' order by xuhao asc";
$rsn =query($sqln); 
$nn=1;
...
$sql="select * from caicaicms_job where id='$id'";
$rs = query($sql); 
$row = fetch_array($rs);
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="1">
                <tr> 
                  <td> <fieldset class="fieldsetstyle">
                    <legend>请选择大类</legend>
                    <?php
        $sqlB = "select classid,classname from caicaicms_jobclass where parentid='0' order by xuhao asc";
		$rsB =query($sqlB); 
		$n=1;
...
echo "<fieldset class='fieldsetstyle'><legend>请选择小类</legend>";
$sqlS="select * from caicaicms_jobclass where parentid='$rowB[classid]' order by xuhao asc";
$rsS =query($sqlS); 
$nn=1;
