...
if ($tablename=="caicaicms_main"){
	if (strpos($id,",")>0){
		$sql="select id,img,flv,editor from caicaicms_main where id in (".$id.")";
	}else{
		$sql="select id,img,flv,editor from caicaicms_main where id ='$id'";
	}
$rs=query($sql);
$row=num_rows($rs);
if ($row){
...
}elseif ($tablename=="caicaicms_pp" || $tablename=="caicaicms_licence"){
...
}elseif ($tablename=='caicaicms_guestbook'){
...
}elseif ($tablename=='caicaicms_dl'){//不是从数据库中删，而是隐藏
if (strpos($id,",")>0){	
	$sql="select id,saver from caicaicms_dl where id in (".$id.")";
}else{	
	$sql="select id,saver from caicaicms_dl where id ='$id'";
}
$rs=query($sql);
$row=num_rows($rs);
if ($row){
while ($row=fetch_array($rs)){	
	...
	query("update caicaicms_dl set del=1 where id =".$row['id']."");
}
echo "<script>location.href='".$pagename."';</script>";
}
