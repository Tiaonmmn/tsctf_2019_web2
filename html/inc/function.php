<?php
define('caicaicmsroot2', str_replace("\\", '/', substr(dirname(__FILE__), 0, -3)));//-3截除当前目录inc
function WriteErrMsg($ErrMsg){
	...
	$strErr=$strErr . "<div style='background-color:#f1f1f1;border-bottom:solid 1px #ddd;font-weight:bold'></div>";
...
function markit(){
		  ...
		  query("insert into caicaicms_bad (username,ip,dose,sendtime)values('".$_COOKIE["UserName"]."','$userip','$url','".date('Y-m-d H:i:s')."')") ;
}
function admindo(){
$adminip=getip();
          $url="http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
		  $f=caicaicmsroot2."admindoes.txt";
...
$str='';
	...
	$n=1;$str='';
	$sql="select title,id,content,sendtime from caicaicms_help where classid=2 and elite=1 order by id desc limit 0,$numbers";
	$rs=query($sql);
	//echo $sql;
	$row=num_rows($rs);
	if ($row){
...
function grabimg($url,$filename="") {
   ...
   if($filename=="") {
	 ...
	 $filename_dir=caicaicmsroot2.'uploadfiles/'.date("Y-m"); //上传文件地址 采用绝对地址方便upload.php文件放在站内的任何位置
...
$fpath=caicaicmsroot2."cache/zskeyword.txt";
}elseif ($channel=='zx'){
$fpath=caicaicmsroot2."cache/zxkeyword.txt";
}
...
if (cache_update_time!=0 && file_exists($fpath)!==false && time()-filemtime($fpath)<3600*24*cache_update_time){
	...
	if ($channel=='zs'||$channel=='dl'){
	$sql= "select keyword,url from caicaicms_tagzs order by xuhao asc";
	}elseif($channel=='zx'){
	$sql= "select keyword,url from caicaicms_tagzx order by xuhao asc";
	}
	$rs=query($sql);
	$row=num_rows($rs);
	if ($row){
...
$fp=caicaicmsroot2."cache/".$siteskin."/adv_".pinyin($b)."_".pinyin($s).".htm";//广告中文类别名转换成拼音字母来给缓存文件命名
...
$n=1;$str='';
//sql= "select * from caicaicms_ad where endtime>= '"&date()&"' "
$sql= "select * from caicaicms_ad where bigclassname='".$b."' and smallclassname='".$s."' order by xuhao asc,id asc ";
...
$str=$str."</ul>";
	...
	$fp=caicaicmsroot2."cache/".$siteskin."/adv_".pinyin($b)."_".pinyin($s).".htm";
	if (!file_exists(caicaicmsroot2."cache/".$siteskin)) {mkdir(caicaicmsroot2."cache/".$siteskin,0777,true);}
...
function lockip(){
$badip=getip();
$sql="select * from caicaicms_bad where ip='".$badip."' and lockip=1";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
echo "此IP被封,不能访问本站!";
...
return "http://".$editor.".".substr(siteurl,strpos(siteurl,".")+1);
	...
	if ($n==2){//http://caicaicms.net的情况
	return "http://".$editor.".".str_replace("http://",'',siteurl);
	}
	if ($n==3){//分两种情况：1 http://demo.caicaicms.net的情况2 http://caicaicms.net.cn
...
if ($channel=='zs'){
$sql="select count(*) as total from caicaicms_main";
}else{
$sql="select count(*) as total from caicaicms_".$channel;
}
$rs=query($sql);
$row = fetch_array($rs);
$totlenum = $row['total'];
...
global $siteskin;
$fpath=caicaicmsroot2."cache/".$siteskin."/sitecount.txt";
if (cache_update_time!=0 && file_exists($fpath)!==false && time()-filemtime($fpath)<3600*24*cache_update_time){
	return file_get_contents($fpath);
}else{	
$str='';
...
$contact=$contact . "<ul>";
$contact=$contact . "<li>";
//$sqln="select groupname,grouppic,groupid,config from caicaicms_usergroup where groupid=(select groupid from caicaicms_user where username=(Select editor From caicaicms_main where id='$cpid'))";
$sqln="select groupname,grouppic,groupid,config from caicaicms_usergroup where groupid=$groupid";
$rsn=query($sqln);
...
$contact=$contact . "<li>";
	if ($kind<>"" && $kind<>0 ) {
	$rsn=query("select classname from caicaicms_userclass where classid=".$kind."");
	$rown=fetch_array($rsn);
...
$rs=query("select config from caicaicms_admingroup where id=(select groupid from caicaicms_admin where pass='".@$_COOKIE["pass"]."' and admin='".@$_COOKIE["admin"]."')");//只验证密码会出现，两个管理员密码相同的情况，导致出错,前加@防止SESSION失效后出错提示
	$row=fetch_array($rs);
	$config=$row["config"];
...
$rs=query("select config from caicaicms_usergroup where groupid=(select groupid from caicaicms_user where username='".$username."')");
	$row=fetch_array($rs);
	$config=$row["config"];
