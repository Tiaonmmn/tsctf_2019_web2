...
$fp=caicaicmsroot."cache/".$siteskin."/zsclass_".$style.".htm";
...
$sql="select * from caicaicms_zsclass where parentid=0 and isshow=1 order by xuhao asc limit 0,$num_b";
$rs=query($sql);
$row=num_rows($rs);
if ($row){
$n=1;
...
$str=$str. "<a href=".getpageurl2("zs",$row["classzm"],'').">".cutstr($row["classname"],$long_b)."</a>";
	if($showcount=='yes'){
	$rsnumb=query("select count(*) as total from caicaicms_main where bigclassid='".$row["classid"]."' ");//统计所属大类下的信息数
...
if ($style==1){//--------------style为1时左侧大类下显示小类
	...
	$rsn=query("select * from caicaicms_zsclass where parentid='".$row["classid"]."' order by xuhao asc limit 0,3");
	$rown=num_rows($rsn);
		$nn=1;
		if ($rown){
...
$nn=1;
$rsn=query("select * from caicaicms_zsclass where parentid='".$row["classid"]."' order by xuhao asc limit 0,$num_s");
$rown=num_rows($rsn);
	if ($rown){
		while ($rown=fetch_array($rsn)){
		...
		$str=$str. "<div class='zsclass_s_name'><a href=".getpageurl2('zs',$row["classzm"],$rown["classzm"]).">".cutstr($rown["classname"],$long_s)."</a></div>\n";
			...
			$sqlcp="select id,proname from caicaicms_main where bigclassid='".$row["classid"]."' and smallclassid in (".$rown['classid'].") order by sendtime desc limit 0,$num_p";
			}else{
			$sqlcp="select id,proname from caicaicms_main where bigclassid='".$row["classid"]."' and smallclassid='".$rown["classid"]."' order by sendtime desc limit 0,$num_p";
			}
			$rscp=query($sqlcp);
			$rowcp=num_rows($rscp);
			if ($rowcp){
...
$str= '暂无分类信息';
	...
	$fp=caicaicmsroot."cache/".$siteskin."/zsclass_".$style.".htm";
	if (!file_exists(caicaicmsroot."cache/".$siteskin)) {mkdir(caicaicmsroot."cache/".$siteskin,0777,true);}
