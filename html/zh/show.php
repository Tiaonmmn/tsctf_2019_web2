...
$sql="select * from caicaicms_zh where id='$zhid'";
$rs=query($sql);
$row=fetch_array($rs);
if (!$row){
echo showmsg("不存在相关信息！");
}else{
query("update caicaicms_zh set hit=hit+1 where id='$zhid'");
$title=$row["title"];
...
$rs=query("select classname from caicaicms_zhclass where classid='".$bigclassid."'");
$row=fetch_array($rs);
if ($row){
