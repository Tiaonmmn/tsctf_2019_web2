...
$smallclassname="";
if ($b<>0){
$sql="select * from caicaicms_zxclass where classid='$b'";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
...
if ($s<>0) {
$sql="select * from caicaicms_zxclass where classid='$s'";
$rs=query($sql);
$row=fetch_array($rs);
if ($row){
...
function formbigclass(){
		$str="";
        $sql = "select * from caicaicms_zxclass where parentid=0";
        $rs=query($sql);
		...
		if ($b<>0){
		$str="";
        $sql="select * from caicaicms_zxclass where parentid='".$b."' order by xuhao asc";
        $rs=query($sql);
			while($row=fetch_array($rs)){
...
$sql="select count(*) as total from caicaicms_zx where passed<>0 ";
$sql2='';
if ($b<>0){
...
$sql="select id,title,elite,sendtime,img,link,hit from caicaicms_zx where passed=1";
$sql=$sql.$sql2;
